...
{
	const gchar* program_class = (const gchar*)asString(s_program_class);
...
S_gdk_pointer_grab(USER_OBJECT_ s_window, USER_OBJECT_ s_owner_events, USER_OBJECT_ s_event_mask, USER_OBJECT_ s_confine_to, USER_OBJECT_ s_cursor, USER_OBJECT_ s_time)
{
	...
	gboolean owner_events = (gboolean)asLogical(s_owner_events);
	GdkEventMask event_mask = (GdkEventMask)asFlag(s_event_mask, GDK_TYPE_EVENT_MASK);
...
{
	guint msec = (guint)asNumeric(s_msec);
...
S_gdk_cairo_set_source_pixbuf(USER_OBJECT_ s_cr, USER_OBJECT_ s_pixbuf, USER_OBJECT_ s_pixbuf_x, USER_OBJECT_ s_pixbuf_y)
{
	...
	double pixbuf_x = (double)asNumeric(s_pixbuf_x);
	double pixbuf_y = (double)asNumeric(s_pixbuf_y);
...
{
	cairo_t* cr = (cairo_t*)getPtrValue(s_cr);
	GdkRectangle* rectangle = asGdkRectangle(s_rectangle);
...
S_gdk_colormap_new(USER_OBJECT_ s_visual, USER_OBJECT_ s_allocate)
{
	...
	gboolean allocate = (gboolean)asLogical(s_allocate);
...
S_gdk_colormap_alloc_color(USER_OBJECT_ s_object, USER_OBJECT_ s_color, USER_OBJECT_ s_writeable, USER_OBJECT_ s_best_match)
{
	...
	GdkColor* color = asGdkColor(s_color);
	gboolean writeable = (gboolean)asLogical(s_writeable);
	gboolean best_match = (gboolean)asLogical(s_best_match);
...
{
	GdkColormap* object = GDK_COLORMAP(getPtrValue(s_object));
	gulong pixel = (gulong)asNumeric(s_pixel);
...
{
	const gchar* spec = (const gchar*)asString(s_spec);
...
{
	GdkCursorType cursor_type = (GdkCursorType)asEnum(s_cursor_type, GDK_TYPE_CURSOR_TYPE);
...
S_gdk_cursor_new_from_pixmap(USER_OBJECT_ s_source, USER_OBJECT_ s_mask, USER_OBJECT_ s_fg, USER_OBJECT_ s_bg, USER_OBJECT_ s_x, USER_OBJECT_ s_y)
{
	...
	GdkColor* fg = asGdkColor(s_fg);
	GdkColor* bg = asGdkColor(s_bg);
...
{
	const gchar* display_name = (const gchar*)asString(s_display_name);
...
{
	GdkDisplay* object = GDK_DISPLAY_OBJECT(getPtrValue(s_object));
	gint screen_num = (gint)asInteger(s_screen_num);
...
{
	GdkDisplay* object = GDK_DISPLAY_OBJECT(getPtrValue(s_object));
	guint32 time_ = (guint32)asNumeric(s_time_);
...
S_gdk_display_add_client_message_filter(USER_OBJECT_ s_object, USER_OBJECT_ s_message_type, USER_OBJECT_ s_func, USER_OBJECT_ s_data)
{
	...
	GdkDisplay* object = GDK_DISPLAY_OBJECT(getPtrValue(s_object));
	GdkAtom message_type = asGdkAtom(s_message_type);
...
S_gdk_display_store_clipboard(USER_OBJECT_ s_object, USER_OBJECT_ s_clipboard_window, USER_OBJECT_ s_targets)
{
	...
	GdkAtom* targets = (GdkAtom*)asGdkAtomArray(s_targets);
...
{
	GdkDisplay* object = GDK_DISPLAY_OBJECT(getPtrValue(s_object));
	guint distance = (guint)asNumeric(s_distance);
...
{
	GdkDragContext* object = GDK_DRAG_CONTEXT(getPtrValue(s_object));
	GdkDragAction action = (GdkDragAction)asFlag(s_action, GDK_TYPE_DRAG_ACTION);
	guint32 time = (guint32)asNumeric(s_time);
...
{
	GdkDragContext* object = GDK_DRAG_CONTEXT(getPtrValue(s_object));
	gboolean ok = (gboolean)asLogical(s_ok);
	guint32 time = (guint32)asNumeric(s_time);
...
{
	GdkDragContext* object = GDK_DRAG_CONTEXT(getPtrValue(s_object));
	gboolean success = (gboolean)asLogical(s_success);
	guint32 time = (guint32)asNumeric(s_time);
...
S_gdk_drag_begin(USER_OBJECT_ s_object, USER_OBJECT_ s_targets)
{
	...
	GList* targets = asGList(s_targets);
...
{
	guint32 xid = (guint32)asNumeric(s_xid);
...
S_gdk_drag_find_window(USER_OBJECT_ s_object, USER_OBJECT_ s_drag_window, USER_OBJECT_ s_x_root, USER_OBJECT_ s_y_root)
{
	...
	gint x_root = (gint)asInteger(s_x_root);
	gint y_root = (gint)asInteger(s_y_root);
...
S_gdk_drag_motion(USER_OBJECT_ s_object, USER_OBJECT_ s_dest_window, USER_OBJECT_ s_protocol, USER_OBJECT_ s_x_root, USER_OBJECT_ s_y_root, USER_OBJECT_ s_suggested_action, USER_OBJECT_ s_possible_actions, USER_OBJECT_ s_time)
{
	...
	gint x_root = (gint)asInteger(s_x_root);
	gint y_root = (gint)asInteger(s_y_root);
	GdkDragAction suggested_action = (GdkDragAction)asFlag(s_suggested_action, GDK_TYPE_DRAG_ACTION);
	GdkDragAction possible_actions = (GdkDragAction)asFlag(s_possible_actions, GDK_TYPE_DRAG_ACTION);
	guint32 time = (guint32)asNumeric(s_time);
...
S_gdk_draw_rectangle(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_filled, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height)
{
	GdkDrawable* object = GDK_DRAWABLE(getPtrValue(s_object));
	GdkGC* gc = GDK_GC(getPtrValue(s_gc));
	gboolean filled = (gboolean)asLogical(s_filled);
	gint x = (gint)asInteger(s_x);
	gint y = (gint)asInteger(s_y);
...
S_gdk_draw_arc(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_filled, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height, USER_OBJECT_ s_angle1, USER_OBJECT_ s_angle2)
{
	...
	gint width = (gint)asInteger(s_width);
	gint height = (gint)asInteger(s_height);
	gint angle1 = (gint)asInteger(s_angle1);
	gint angle2 = (gint)asInteger(s_angle2);
...
S_gdk_draw_text(USER_OBJECT_ s_object, USER_OBJECT_ s_font, USER_OBJECT_ s_gc, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_text, USER_OBJECT_ s_text_length)
{
	...
	gint text_length = (gint)asInteger(s_text_length);
...
S_gdk_draw_text_wc(USER_OBJECT_ s_object, USER_OBJECT_ s_font, USER_OBJECT_ s_gc, USER_OBJECT_ s_x, USER_OBJECT_ s_text)
{
	...
	const GdkWChar* text = (const GdkWChar*)asArray(s_text, GdkWChar, asNumeric);
...
S_gdk_draw_drawable(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_src, USER_OBJECT_ s_xsrc, USER_OBJECT_ s_ysrc, USER_OBJECT_ s_xdest, USER_OBJECT_ s_ydest, USER_OBJECT_ s_width, USER_OBJECT_ s_height)
{
	...
	gint xsrc = (gint)asInteger(s_xsrc);
	gint ysrc = (gint)asInteger(s_ysrc);
	gint xdest = (gint)asInteger(s_xdest);
	gint ydest = (gint)asInteger(s_ydest);
	gint width = (gint)asInteger(s_width);
	gint height = (gint)asInteger(s_height);
...
S_gdk_draw_segments(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_segs)
{
	GdkDrawable* object = GDK_DRAWABLE(getPtrValue(s_object));
	GdkGC* gc = GDK_GC(getPtrValue(s_gc));
	GdkSegment* segs = (GdkSegment*)asArrayRef(s_segs, GdkSegment, asGdkSegment);
...
S_gdk_draw_pixbuf(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_pixbuf, USER_OBJECT_ s_src_x, USER_OBJECT_ s_src_y, USER_OBJECT_ s_dest_x, USER_OBJECT_ s_dest_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height, USER_OBJECT_ s_dither, USER_OBJECT_ s_x_dither, USER_OBJECT_ s_y_dither)
{
	...
	gint dest_x = (gint)asInteger(s_dest_x);
	gint dest_y = (gint)asInteger(s_dest_y);
	gint width = (gint)asInteger(s_width);
	gint height = (gint)asInteger(s_height);
	GdkRgbDither dither = (GdkRgbDither)asEnum(s_dither, GDK_TYPE_RGB_DITHER);
	gint x_dither = (gint)asInteger(s_x_dither);
	gint y_dither = (gint)asInteger(s_y_dither);
...
S_gdk_draw_layout_line_with_colors(USER_OBJECT_ s_drawable, USER_OBJECT_ s_gc, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_line, USER_OBJECT_ s_foreground, USER_OBJECT_ s_background)
{
	...
	gint y = (gint)asInteger(s_y);
	PangoLayoutLine* line = (PangoLayoutLine*)getPtrValue(s_line);
	GdkColor* foreground = asGdkColor(s_foreground);
	GdkColor* background = asGdkColor(s_background);
...
S_gdk_draw_trapezoids(USER_OBJECT_ s_drawable, USER_OBJECT_ s_gc, USER_OBJECT_ s_trapezoids)
{
	GdkDrawable* drawable = GDK_DRAWABLE(getPtrValue(s_drawable));
	GdkGC* gc = GDK_GC(getPtrValue(s_gc));
	GdkTrapezoid* trapezoids = (GdkTrapezoid*)asArrayRef(s_trapezoids, GdkTrapezoid, asGdkTrapezoid);
...
{
	GdkEventType type = (GdkEventType)asEnum(s_type, GDK_TYPE_EVENT_TYPE);
...
{
	GdkEvent* object = (GdkEvent*)getPtrValue(s_object);
	GdkAxisUse axis_use = (GdkAxisUse)asEnum(s_axis_use, GDK_TYPE_AXIS_USE);
...
{
	gboolean show_events = (gboolean)asLogical(s_show_events);
...
{
	GdkDisplay* display = GDK_DISPLAY_OBJECT(getPtrValue(s_display));
	const gchar* font_name = (const gchar*)asString(s_font_name);
...
{
	GdkDisplay* display = GDK_DISPLAY_OBJECT(getPtrValue(s_display));
	const gchar* fontset_name = (const gchar*)asString(s_fontset_name);
...
{
	GdkFont* object = (GdkFont*)getPtrValue(s_object);
	gchar character = (gchar)asCharacter(s_character);
...
{
	GdkFont* object = (GdkFont*)getPtrValue(s_object);
	GdkWChar character = (GdkWChar)asNumeric(s_character);
...
{
	GdkGC* object = GDK_GC(getPtrValue(s_object));
	GdkFunction function = (GdkFunction)asEnum(s_function, GDK_TYPE_FUNCTION);
...
{
	GdkGC* object = GDK_GC(getPtrValue(s_object));
	GdkFill fill = (GdkFill)asEnum(s_fill, GDK_TYPE_FILL);
...
{
	GdkGC* object = GDK_GC(getPtrValue(s_object));
	GdkSubwindowMode mode = (GdkSubwindowMode)asEnum(s_mode, GDK_TYPE_SUBWINDOW_MODE);
...
{
	GdkGC* object = GDK_GC(getPtrValue(s_object));
	gboolean exposures = (gboolean)asLogical(s_exposures);
...
{
	GdkGC* object = GDK_GC(getPtrValue(s_object));
	gint line_width = (gint)asInteger(s_line_width);
	GdkLineStyle line_style = (GdkLineStyle)asEnum(s_line_style, GDK_TYPE_LINE_STYLE);
	GdkCapStyle cap_style = (GdkCapStyle)asEnum(s_cap_style, GDK_TYPE_CAP_STYLE);
	GdkJoinStyle join_style = (GdkJoinStyle)asEnum(s_join_style, GDK_TYPE_JOIN_STYLE);
...
S_gdk_gc_set_dashes(USER_OBJECT_ s_object, USER_OBJECT_ s_dash_list)
{
	...
	gint8* dash_list = (gint8*)asArray(s_dash_list, gint8, asInteger);
...
{
	GdkGC* object = GDK_GC(getPtrValue(s_object));
	gint x_offset = (gint)asInteger(s_x_offset);
	gint y_offset = (gint)asInteger(s_y_offset);
...
{
	GdkImageType type = (GdkImageType)asEnum(s_type, GDK_TYPE_IMAGE_TYPE);
	GdkVisual* visual = GDK_VISUAL(getPtrValue(s_visual));
	gint width = (gint)asInteger(s_width);
	gint height = (gint)asInteger(s_height);
...
S_gdk_image_put_pixel(USER_OBJECT_ s_object, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_pixel)
{
	...
	gint x = (gint)asInteger(s_x);
	gint y = (gint)asInteger(s_y);
	guint32 pixel = (guint32)asNumeric(s_pixel);
...
S_gdk_device_set_source(USER_OBJECT_ s_object, USER_OBJECT_ s_source)
{
	...
	GdkInputSource source = (GdkInputSource)asEnum(s_source, GDK_TYPE_INPUT_SOURCE);
...
{
	GdkDevice* object = GDK_DEVICE(getPtrValue(s_object));
	GdkInputMode mode = (GdkInputMode)asEnum(s_mode, GDK_TYPE_INPUT_MODE);
...
S_gdk_device_set_axis_use(USER_OBJECT_ s_object, USER_OBJECT_ s_index, USER_OBJECT_ s_use)
{
	GdkDevice* object = GDK_DEVICE(getPtrValue(s_object));
	guint index = (guint)asNumeric(s_index);
	GdkAxisUse use = (GdkAxisUse)asEnum(s_use, GDK_TYPE_AXIS_USE);
...
{
	GdkDevice* object = GDK_DEVICE(getPtrValue(s_object));
	gdouble* axes = (gdouble*)asArray(s_axes, gdouble, asNumeric);
	GdkAxisUse use = (GdkAxisUse)asEnum(s_use, GDK_TYPE_AXIS_USE);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	gint mask = (gint)asInteger(s_mask);
...
S_gdk_keymap_lookup_key(USER_OBJECT_ s_object, USER_OBJECT_ s_key)
{
	...
	const GdkKeymapKey* key = asGdkKeymapKey(s_key);
...
{
	GdkKeymap* object = GDK_KEYMAP(getPtrValue(s_object));
	guint hardware_keycode = (guint)asNumeric(s_hardware_keycode);
	GdkModifierType state = (GdkModifierType)asFlag(s_state, GDK_TYPE_MODIFIER_TYPE);
	gint group = (gint)asInteger(s_group);
...
{
	const gchar* keyval_name = (const gchar*)asString(s_keyval_name);
...
{
	guint symbol = (guint)asNumeric(s_symbol);
...
{
	guint32 wc = (guint32)asNumeric(s_wc);
...
S_gdk_pango_renderer_set_override_color(USER_OBJECT_ s_object, USER_OBJECT_ s_part, USER_OBJECT_ s_color)
{
	GdkPangoRenderer* object = GDK_PANGO_RENDERER(getPtrValue(s_object));
	PangoRenderPart part = (PangoRenderPart)asEnum(s_part, PANGO_TYPE_RENDER_PART);
	const GdkColor* color = GET_LENGTH(s_color) == 0 ? NULL : asGdkColor(s_color);
...
{
	PangoLayoutLine* line = (PangoLayoutLine*)getPtrValue(s_line);
	gint x_origin = (gint)asInteger(s_x_origin);
	gint y_origin = (gint)GET_LENGTH(s_index_ranges);
	gint* index_ranges = (gint*)asArray(s_index_ranges, gint, asInteger);
...
{
	gboolean embossed = (gboolean)asLogical(s_embossed);
...
S_gdk_pixbuf_render_to_drawable_alpha(USER_OBJECT_ s_object, USER_OBJECT_ s_drawable, USER_OBJECT_ s_src_x, USER_OBJECT_ s_src_y, USER_OBJECT_ s_dest_x, USER_OBJECT_ s_dest_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height, USER_OBJECT_ s_alpha_mode, USER_OBJECT_ s_alpha_threshold, USER_OBJECT_ s_dither, USER_OBJECT_ s_x_dither, USER_OBJECT_ s_y_dither)
{
	...
	int width = (int)asInteger(s_width);
	int height = (int)asInteger(s_height);
	GdkPixbufAlphaMode alpha_mode = (GdkPixbufAlphaMode)asEnum(s_alpha_mode, GDK_TYPE_PIXBUF_ALPHA_MODE);
...
{
	GdkDrawable* drawable = GDK_DRAWABLE(getPtrValue(s_drawable));
	GdkColor* transparent_color = asGdkColor(s_transparent_color);
...
S_gdk_pixmap_create_from_xpm_d(USER_OBJECT_ s_drawable, USER_OBJECT_ s_transparent_color, USER_OBJECT_ s_data)
{
	GdkDrawable* drawable = GDK_DRAWABLE(getPtrValue(s_drawable));
	GdkColor* transparent_color = asGdkColor(s_transparent_color);
	gchar** data = (gchar**)asStringArray(s_data);
...
{
	GdkNativeWindow anid = asGdkNativeWindow(s_anid);
...
USER_OBJECT_
S_gdk_atom_name(USER_OBJECT_ s_name)
{
	GdkAtom name = asGdkAtom(s_name);

	gchar* ans;
	USER_OBJECT_ _result = NULL_USER_OBJECT;

	ans = gdk_atom_name(name);
...
{
	gchar* atom_name = (gchar*)asString(s_atom_name);
	gboolean only_if_exists = (gboolean)asLogical(s_only_if_exists);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	GdkAtom property = asGdkAtom(s_property);
	GdkAtom type = asGdkAtom(s_type);
	gulong offset = (gulong)asNumeric(s_offset);
	gulong length = (gulong)asNumeric(s_length);
	gint pdelete = (gint)asInteger(s_pdelete);
...
S_gdk_property_change(USER_OBJECT_ s_object, USER_OBJECT_ s_property, USER_OBJECT_ s_type, USER_OBJECT_ s_format, USER_OBJECT_ s_mode, USER_OBJECT_ s_data)
{
	...
	GdkAtom property = asGdkAtom(s_property);
	GdkAtom type = asGdkAtom(s_type);
	gint format = (gint)asInteger(s_format);
	GdkPropMode mode = (GdkPropMode)asEnum(s_mode, GDK_TYPE_PROP_MODE);
	const guchar* data = (const guchar*)asArray(s_data, guchar, asInteger);
...
{
	guint32 rgb = (guint32)asNumeric(s_rgb);
...
USER_OBJECT_
S_gdk_draw_rgb_image(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height, USER_OBJECT_ s_dith, USER_OBJECT_ s_rgb_buf)
{
	GdkDrawable* object = GDK_DRAWABLE(getPtrValue(s_object));
	GdkGC* gc = GDK_GC(getPtrValue(s_gc));
	gint x = (gint)asInteger(s_x);
	...
	gint width = (gint)asInteger(s_width);
	gint height = (gint)asInteger(s_height);
	GdkRgbDither dith = (GdkRgbDither)asEnum(s_dith, GDK_TYPE_RGB_DITHER);
	guchar* rgb_buf = (guchar*)asArray(s_rgb_buf, guchar, asInteger);
...
S_gdk_draw_rgb_image_dithalign(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height, USER_OBJECT_ s_dith, USER_OBJECT_ s_rgb_buf, USER_OBJECT_ s_xdith, USER_OBJECT_ s_ydith)
{
	...
	guchar* rgb_buf = (guchar*)asArray(s_rgb_buf, guchar, asInteger);
	gint rowstride = (gint)GET_LENGTH(s_rgb_buf);
	gint xdith = (gint)asInteger(s_xdith);
	gint ydith = (gint)asInteger(s_ydith);
...
S_gdk_draw_rgb_32_image(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height, USER_OBJECT_ s_dith, USER_OBJECT_ s_buf)
{
	...
	gint height = (gint)asInteger(s_height);
	GdkRgbDither dith = (GdkRgbDither)asEnum(s_dith, GDK_TYPE_RGB_DITHER);
	guchar* buf = (guchar*)asArray(s_buf, guchar, asInteger);
...
{
	guint32* colors = (guint32*)asArray(s_colors, guint32, asNumeric);
...
S_gdk_draw_indexed_image(USER_OBJECT_ s_object, USER_OBJECT_ s_gc, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_width, USER_OBJECT_ s_height, USER_OBJECT_ s_dith, USER_OBJECT_ s_buf, USER_OBJECT_ s_cmap)
{
	...
	guchar* buf = (guchar*)asArray(s_buf, guchar, asInteger);
	gint rowstride = (gint)GET_LENGTH(s_buf);
	GdkRgbCmap* cmap = asGdkRgbCmap(s_cmap);
...
{
	gboolean verbose = (gboolean)asLogical(s_verbose);
...
{
	gboolean install = (gboolean)asLogical(s_install);
...
{
	gint min_colors = (gint)asInteger(s_min_colors);
...
S_gdk_selection_owner_set(USER_OBJECT_ s_owner, USER_OBJECT_ s_selection, USER_OBJECT_ s_time, USER_OBJECT_ s_send_event)
{
	...
	GdkAtom selection = asGdkAtom(s_selection);
	guint32 time = (guint32)asNumeric(s_time);
	gboolean send_event = (gboolean)asLogical(s_send_event);
...
{
	guint32 requestor = (guint32)asNumeric(s_requestor);
	GdkAtom selection = asGdkAtom(s_selection);
	GdkAtom target = asGdkAtom(s_target);
	GdkAtom property = asGdkAtom(s_property);
	guint32 time = (guint32)asNumeric(s_time);
...
{
	GdkVisualType visual_type = (GdkVisualType)asEnum(s_visual_type, GDK_TYPE_VISUAL_TYPE);
...
S_gdk_visual_get_best_with_both(USER_OBJECT_ s_depth, USER_OBJECT_ s_visual_type)
{
	...
	_result = toRPointerWithFinalizer(ans, "GdkVisual", (RPointerFinalizer) g_object_unref);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	gboolean override_redirect = (gboolean)asLogical(s_override_redirect);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	gint dx = (gint)asInteger(s_dx);
	gint dy = (gint)asInteger(s_dy);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	gboolean use_static = (gboolean)asLogical(s_use_static);
...
S_gdk_window_set_hints(USER_OBJECT_ s_object, USER_OBJECT_ s_x, USER_OBJECT_ s_y, USER_OBJECT_ s_min_width, USER_OBJECT_ s_min_height, USER_OBJECT_ s_max_width, USER_OBJECT_ s_max_height, USER_OBJECT_ s_flags)
{
	...
	gint max_height = (gint)asInteger(s_max_height);
	gint flags = (gint)asInteger(s_flags);
...
{
	const gchar* sm_client_id = (const gchar*)asString(s_sm_client_id);
...
S_gdk_window_set_back_pixmap(USER_OBJECT_ s_object, USER_OBJECT_ s_pixmap, USER_OBJECT_ s_parent_relative)
{
	...
	gboolean parent_relative = (gboolean)asLogical(s_parent_relative);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	GList* pixbufs = (GList*)asArrayRef(s_pixbufs, GList, asGList);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	GdkWMDecoration decorations = (GdkWMDecoration)asFlag(s_decorations, GDK_TYPE_WM_DECORATION);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	GdkWMFunction functions = (GdkWMFunction)asFlag(s_functions, GDK_TYPE_WM_FUNCTION);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	GdkRectangle* rect = GET_LENGTH(s_rect) == 0 ? NULL : asGdkRectangle(s_rect);
	gboolean invalidate_children = (gboolean)asLogical(s_invalidate_children);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	gboolean update_children = (gboolean)asLogical(s_update_children);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	gboolean accept_focus = (gboolean)asLogical(s_accept_focus);
...
{
	GdkWindow* object = GDK_WINDOW(getPtrValue(s_object));
	gboolean focus_on_map = (gboolean)asLogical(s_focus_on_map);
...
S_gtk_drag_finish(USER_OBJECT_ s_object, USER_OBJECT_ s_success, USER_OBJECT_ s_del, USER_OBJECT_ s_time)
{
	GdkDragContext* object = GDK_DRAG_CONTEXT(getPtrValue(s_object));
	gboolean success = (gboolean)asLogical(s_success);
	gboolean del = (gboolean)asLogical(s_del);
	guint32 time = (guint32)asNumeric(s_time);
...
{
	GdkColorspace colorspace = (GdkColorspace)asEnum(s_colorspace, GDK_TYPE_COLORSPACE);
	gboolean has_alpha = (gboolean)asLogical(s_has_alpha);
	int bits_per_sample = (int)asInteger(s_bits_per_sample);
	int width = (int)asInteger(s_width);
	int height = (int)asInteger(s_height);
...
{
	const char** data = (const char**)asStringArray(s_data);
...
S_gdk_pixbuf_new_from_inline(USER_OBJECT_ s_data, USER_OBJECT_ s_copy_pixels)
{
	...
	const guchar* data = (const guchar*)asArray(s_data, guchar, asInteger);
	gboolean copy_pixels = (gboolean)asLogical(s_copy_pixels);
...
S_gdk_pixbuf_savev(USER_OBJECT_ s_object, USER_OBJECT_ s_filename, USER_OBJECT_ s_type, USER_OBJECT_ s_option_keys, USER_OBJECT_ s_option_values)
{
	GdkPixbuf* object = GDK_PIXBUF(getPtrValue(s_object));
	const char* filename = (const char*)asString(s_filename);
	const char* type = (const char*)asString(s_type);
	char** option_keys = (char**)asStringArray(s_option_keys);
	char** option_values = (char**)asStringArray(s_option_values);
...
{
	GdkPixbuf* object = GDK_PIXBUF(getPtrValue(s_object));
	gboolean substitute_color = (gboolean)asLogical(s_substitute_color);
	guchar r = (guchar)asInteger(s_r);
	guchar g = (guchar)asInteger(s_g);
	guchar b = (guchar)asInteger(s_b);
...
S_gdk_pixbuf_saturate_and_pixelate(USER_OBJECT_ s_object, USER_OBJECT_ s_dest, USER_OBJECT_ s_saturation, USER_OBJECT_ s_pixelate)
{
	...
	gfloat saturation = (gfloat)asNumeric(s_saturation);
	gboolean pixelate = (gboolean)asLogical(s_pixelate);
...
S_gdk_pixbuf_scale(USER_OBJECT_ s_object, USER_OBJECT_ s_dest, USER_OBJECT_ s_dest_x, USER_OBJECT_ s_dest_y, USER_OBJECT_ s_dest_width, USER_OBJECT_ s_dest_height, USER_OBJECT_ s_offset_x, USER_OBJECT_ s_offset_y, USER_OBJECT_ s_scale_x, USER_OBJECT_ s_scale_y, USER_OBJECT_ s_interp_type)
{
	...
	int dest_x = (int)asInteger(s_dest_x);
	int dest_y = (int)asInteger(s_dest_y);
	int dest_width = (int)asInteger(s_dest_width);
	int dest_height = (int)asInteger(s_dest_height);
	double offset_x = (double)asNumeric(s_offset_x);
	double offset_y = (double)asNumeric(s_offset_y);
	double scale_x = (double)asNumeric(s_scale_x);
	double scale_y = (double)asNumeric(s_scale_y);
	GdkInterpType interp_type = (GdkInterpType)asEnum(s_interp_type, GDK_TYPE_INTERP_TYPE);
...
S_gdk_pixbuf_composite(USER_OBJECT_ s_object, USER_OBJECT_ s_dest, USER_OBJECT_ s_dest_x, USER_OBJECT_ s_dest_y, USER_OBJECT_ s_dest_width, USER_OBJECT_ s_dest_height, USER_OBJECT_ s_offset_x, USER_OBJECT_ s_offset_y, USER_OBJECT_ s_scale_x, USER_OBJECT_ s_scale_y, USER_OBJECT_ s_interp_type, USER_OBJECT_ s_overall_alpha)
{
	...
	double scale_y = (double)asNumeric(s_scale_y);
	GdkInterpType interp_type = (GdkInterpType)asEnum(s_interp_type, GDK_TYPE_INTERP_TYPE);
	int overall_alpha = (int)asInteger(s_overall_alpha);
...
S_gdk_pixbuf_composite_color(USER_OBJECT_ s_object, USER_OBJECT_ s_dest, USER_OBJECT_ s_dest_x, USER_OBJECT_ s_dest_y, USER_OBJECT_ s_dest_width, USER_OBJECT_ s_dest_height, USER_OBJECT_ s_offset_x, USER_OBJECT_ s_offset_y, USER_OBJECT_ s_scale_x, USER_OBJECT_ s_scale_y, USER_OBJECT_ s_interp_type, USER_OBJECT_ s_overall_alpha, USER_OBJECT_ s_check_x, USER_OBJECT_ s_check_y, USER_OBJECT_ s_check_size, USER_OBJECT_ s_color1, USER_OBJECT_ s_color2)
{
	...
	GdkInterpType interp_type = (GdkInterpType)asEnum(s_interp_type, GDK_TYPE_INTERP_TYPE);
	int overall_alpha = (int)asInteger(s_overall_alpha);
	int check_x = (int)asInteger(s_check_x);
	int check_y = (int)asInteger(s_check_y);
	int check_size = (int)asInteger(s_check_size);
	guint32 color1 = (guint32)asNumeric(s_color1);
	guint32 color2 = (guint32)asNumeric(s_color2);
...
{
	GdkPixbufAnimation* object = GDK_PIXBUF_ANIMATION(getPtrValue(s_object));
	const GTimeVal* start_time = asGTimeVal(s_start_time);
...
{
	GdkPixbufAnimationIter* object = GDK_PIXBUF_ANIMATION_ITER(getPtrValue(s_object));
	const GTimeVal* current_time = asGTimeVal(s_current_time);
...
{
	GdkPixbuf* object = GDK_PIXBUF(getPtrValue(s_object));
	const char* key = (const char*)asString(s_key);
...
{
	const char* image_type = (const char*)asString(s_image_type);
...
S_gdk_pixbuf_loader_write(USER_OBJECT_ s_object, USER_OBJECT_ s_buf)
{
	...
	const guchar* buf = (const guchar*)asArray(s_buf, guchar, asInteger);
...
{
	GdkRectangle* src1 = asGdkRectangle(s_src1);
	GdkRectangle* src2 = asGdkRectangle(s_src2);
