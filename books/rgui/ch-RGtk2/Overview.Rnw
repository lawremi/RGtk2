...
event loop of \GTK. This sits idle until some event
occurs. \pkg{RGtk2} treats the R event loop as the master and
interates the \GTK\/ event loop whenever R is idle. 
...
\function{ok_button_clicked} function as the handler for the
