...
listFiles <- function() {
  ...
  nms <- sub(".R$", "", nms)
  nms <- gsub("-", " ", nms)
  return(data.frame(names=nms, files=files, stringsAsFactors=FALSE))
}
...
b <- gtkToolButton(); quitAction$connectProxy(b)
tb$add(b)
...
b <- gtkToolButton(); runBlockAction$connectProxy(b)
tb$add(b)
...
b <- gtkToolButton(); runSelectedAction$connectProxy(b)
tb$add(b)
...
b <- gtkToolButton(); runAction$connectProxy(b)
tb$add(b)
