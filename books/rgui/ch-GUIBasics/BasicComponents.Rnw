...
but is less usable if the sequence is long. A spin button is used in
...
selections gets bigger than 30. Additionally, list boxes can be
...
each column has a specific data type and cell rendering strategy. The
table widget need not behave like a spread sheet, despite similarity
in appearance. For example, in \GTK\/ the selection is done by row, as
in a list box.

Table widgets may be used to display and select tabular data, or may
also allow the user to edit the
data. Figure~\ref{fig:GUI:table-widgets} shows the data editor from
the \pkg{JGR} GUI that uses Java's table widget.

In \GTK\/ and Java/Swing, the table widget has a separation of
...
action.  Well chosen icons make a big visual difference in a GUI. The
KDE print dialog, contrasted to the Firefox print dialog in
Figure~\ref{fig:GUI:print-dialogs} illustrates this point.
...
more information can be gather to complete the action, as opposed to
...
O\lther common choosers are color choosers and font choosers.
