...
a graphical user interface (GUI). A GUI is a collection of objects called
widgets. A widget is a graphical element on a computer display that
communicates some information to the user and possibly accepts input from the user. 
\pkg{RGtk2} is a programming interface or set of bindings between 
...
XML). The primary goal of \pkg{RGtk2} is programmatic access to \pkg{GTK+} and 
...
low-level mapping of \proglang{C} functions and data structures of each API to 
\proglang{R}. 
\pkg{RGtk2} allows the user to create and extend \pkg{GObject} classes from \proglang{R}. 
...
GNU Image Manipulation Program. \pkg{RGtk2} is based on \pkg{RGtk}, which is
an interface between \proglang{R} and \pkg{GTK+} 1.2. \pkg{RGtk2} interfaces 
with the later series of \pkg{GTK+}, version 2.0. \pkg{GTK+} 2.0 is 
integrated with several other libraries: \pkg{Pango} \citep{pango} 
for fonts and text layout, \pkg{Cairo} \citep{cairo}
for vector-based drawing, \pkg{ATK} \citep{atk} for accessibility
and \pkg{GdkPixbuf} \citep{gdkpixbuf} for images. \pkg{RGtk2} binds to these
supporting libraries in addition to \pkg{GTK+} itself. 
The \pkg{gtkDevice}
\citep{gtkDevice} and \pkg{cairoDevice} \citep{cairoDevice} packages allow
the embedding of \proglang{R} graphics into \pkg{GTK+} interfaces.
...
to invent new types of objects. This means that \pkg{RGtk2} is a low-level
interface to the \proglang{C} libraries, and it is expected that other
packages will build on \pkg{RGtk2} to provide higher-level interfaces. Second, 
interaction with \pkg{RGtk2} must
