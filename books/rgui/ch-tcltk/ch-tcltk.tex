...
widget. These states can be adjusted throug the \code{state} command
...
To use the image, one specifird the image name to the
...
using \code{\event{Return}}. In the following we bind the keypresses to the
...
The following trivial examplesl illustrates, whereas
...
involved, than with \pkg{RGtk2} and \pkg{qtbase}. Steps are needed to
...
\code{\VirtualEvent{DragLeave}} and most importantly \code{\VirtualEvent{DragDrop}} for the
widget we are over.
\begin{Schunk}
\begin{Sinput}
...
These commands are illlustrated below, where we show how one might
...
new style for a button (\code{Toolbar.TButton}), slightly modifiying the themed button to set
...
layout. (No such widget is standard with \pkg{tcltk}.) The following relies on
the functions \function{day.of.week} (from the \pkg{chron} package),
...
\method{add}{ttkpandedwindow} command is used to add a child component. For example:
\begin{Schunk}
\begin{Sinput}
...
returns a \code{tclObj} object. The value is empty when there is no selection made.
...
The \constructor{ttkcombobox} constructor returns a combobox object
allowing for
...
8.5. The constructor \constructor{tkscale}, for a non-themable slider,
...
or not select a value (\qcode{disabled}).  As with a combobox, when
...
a mean. For this purpose, the use is straighforward.
\begin{Schunk}
\begin{Sinput}
 putLabel(lf, "mu:", 2, 0)
...
key activity. The \%-substition \code{k} gives the keycode and
...
\code{as.character} works much better at extracting the individaul
