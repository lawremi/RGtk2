...
operator, but \verb+[[+ and \code{get} will also work. (With the
...
top, bottom, etc. In \pkg{qtbase} these enumerations are stored in
...
operator \verb+|+ to combine values. For example, aligning text in
label in the upper right can be done through
...
ones are \verb+Qt$QPalette$Highlight+, to indicate a selected item, or
\verb+Qt$QPalette$WindowText+ to indicate a foreground color.
