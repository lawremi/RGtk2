...
##
sapply(views, function(view) view$getSelection()$setMode('multiple'))
@
...
make_view_column <- function() {
  ...
  column$addAttribute(gtkCellRendererPixbuf(), "pixbuf", 1L)
  column$addAttribute(gtkCellRendererText(), "text", 0L)
  column
}
sapply(views, function(view) view$insertColumn(make_view_column(), 0))
@ 
