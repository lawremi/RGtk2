...
thus is usually called a \textit{container}. This design is natural,
...
\textit{layout manager} that dynamically calculates the layout as
...
The expanding box will show or hide its children, according to the
state of a toggle button. By way of analogy, radio buttons are to
notebooks as check buttons are to expanding boxes. An expanding box
