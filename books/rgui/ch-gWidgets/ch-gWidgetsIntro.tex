...
\code{txtPattern}, the main property is the text, for the button and
...
%% ML: does it really make sense only to consider the options when
%% they are visible? What happens if they set them, then hide
%% them. It would be surprising to me if they were then ignored.

%% Fair enough
We modify our button handler so that it uses these values, if
specified.

%% We only do so if this part of the GUI is disclosed, by
