...
There are five selection modes for item views defined by the
...
enumeration. Table~\ref{tab:qt:enum:itemDataRole} selectively lists
this enumeration.
...
cursor$insertImage(icon$pixmap(icon$actualSize(qsize(32L,32L)))$toImage())
@ 
%%
...
\qcode{EndOfWord} etc. For example, to move the cursor to the start of
the second line, we could do:
<<results=hide>>=
cursor <- te$textCursor()
cursor$movePosition(Qt$QTextCursor$Start) # MoveAnchor is default
...
cursor$movePosition(Qt$QTextCursor$WordRight, 
...
\signal{selectionChanged} when the selection (according to the user
visible cursor) changes. For the latter, the \signal{copyAvailable}
