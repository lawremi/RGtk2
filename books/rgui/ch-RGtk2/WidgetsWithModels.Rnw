...
gSignalConnect(selection, "changed", function(selection) {
  ...
  if(length(selected_rows$retval)) {
    rows <- sapply(selected_rows$retval, gtkTreePathGetIndices) + 1L
...
cell_renderer <- gtkCellRendererText()
view$insertColumnWithAttributes(0, "numbers", cell_renderer, text = 0)
column <- view$getColumn(0)
...
by(Cars93, Cars93$Manufacturer, function(df) {
  ...
  sapply(df$Model, function(car_model) {
    child_iter <- tstore$append(parent = parent_iter$iter)
    if (is.null(child_iter$retval)) 
      model$setValue(child_iter$iter, column = 0, value = car_model)
  })
})
@ 
...
buffer$applyTag(tag_bold, iter$start, iter$end)  # updates iters
