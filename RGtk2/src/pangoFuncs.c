...
S_pango_color_parse(USER_OBJECT_ s_spec)
{
	...
	gboolean ans;
	USER_OBJECT_ _result = NULL_USER_OBJECT;
	PangoColor* color = (PangoColor *)g_new0(PangoColor, 1);
...
S_pango_parse_markup(USER_OBJECT_ s_markup_text, USER_OBJECT_ s_length, USER_OBJECT_ s_accel_marker)
{
	...
	_result = retByVal(_result, "attr_list", toRPointerWithFinalizer(attr_list, "PangoAttrList", (RPointerFinalizer) pango_attr_list_unref), "text", asRString(text), "accel_char", asRNumeric(accel_char), "error", asRGError(error), NULL);
...
S_pango_coverage_ref(USER_OBJECT_ s_object)
{
	...
	_result = toRPointer(ans, "PangoCoverage");
...
S_pango_coverage_copy(USER_OBJECT_ s_object)
{
	...
	_result = toRPointerWithFinalizer(ans, "PangoCoverage", (RPointerFinalizer) g_free);
...
S_pango_layout_get_font_description(USER_OBJECT_ s_object)
{
	...
	_result = toRPointer(ans, "PangoFontDescription");
...
S_pango_renderer_get_matrix(USER_OBJECT_ s_object)
{
	...
	_result = toRPointer(ans, "PangoMatrix");
