...
should be passed to qconnect. For example, the following two calls are
equivalent:
...
class using the \function{qsetProperty} function. Here is the simplest
usage:
...
system. That requires specifying the \argument{type}{qsetProperty}
argument, which we will cover later. 
...
<<qt-overview-define-prop-getter-setter>>=
...
pass the name of the signal as the \argument{notify}{qsetProperty} of
\function{qsetProperty}:
<<qt-overview-define-prop-notify>>=
...
\pkg{QtDesigner} is a tool for graphical, drag-and-drop design of
GUI forms. Although this book focuses on constructing a GUI by
programming in R, we recognize that a graphical approach may be 
preferable in some circumstances.  \pkg{QtDesigner} outputs a GUI
definition as an XML file in the \qcode{UI} format. The
\class{QUiLoader} class loads a \qcode{UI} definition through its
\method{load}{QUiLoader} method:
...
and corresponding text entry ``findEntry'' in our UI file, then we
retrieve them with
...
@ 
%
Then we connect to the \signal{clicked}{QPushButton} signal:
...
<<qt-overview-designer-custom-parent, eavl=FALSE>>=
qsetClass("MyMainWindow", Qt$QWidget, function() {
  Qt$QMetaObject$connectSlotsByName(this)
})
@ 
%
The constructor calls \method{connectSlotsByName}{QMetaObject} to
automatically establish the connections. For a slot to be connected to
the correct signal, it must be named according to the convention
\qcode{on_[objectName]_[signalName]}. For example,
