...
\qcode{Fixed} (\code{Qt\$QSizePolicy\$Fixed}) for using the size hint;
...
specifies values \qcode{Left} (\code{Qt\$Qt\$AlignLeft}), \qcode{Right},
...
as \code{Qt\$QTabWidget\$North}. Call \method{isMovable}{QTabWidget}
...
the callback a boolean for checked which applies for buttons.
