...
built with \software{QtDesigner}, see Section~\ref{sec:qt-designer}. 
...
class. The \function{qsetProperty} function defines a property. This
is the simplest usage:
<<qt-overview-define-prop>>=
...
of the incoming value. One might also want to override the
...
\function{qproperties} function. To force a property to be read-only,
we could set the \argument{write}{qsetProperty} parameter to
\qcode{NULL}.
...
programming in R, we recognize that a graphical approach may be the
...
The \qcode{widget} object could be shown directly; however, we first
...
the widget with the signal.  Every widget in a \qcode{UI} file is
