...
\igui{data frame models}
The model, view, controller (MVC) pattern\iprogram{model-view-controller} is fundamental to the design
of widgets that display and manipulate data. Keeping the model
separate from the view allows multiple views for the same
data. Generally, the model is an abstract \iprogram{interface}interface. Thus, the same
...
\class{QAbstractItemModel} instance. In \R, the canonical structure for tabular
...
\begin{figure}
  \centering
  \includegraphics[width=.8\textwidth]{fig-qt-tableview-basic}
  \caption{Basic display of a data frame using just three commands}
  \label{fig:qt-tableview-basic}
\end{figure}


Displaying a simple table of data with \class{DataFrameModel} is much
simpler than with \GTK{} and \class{RGtkDataFrame}.
Here we construct a widget to show a
\class{data.frame} in a table view:
<<qt-mvc-qdfm,print=false, results=hide>>=
model <- qdataFrameModel(mtcars)
view <- Qt$QTableView()
view$setModel(model)
@ 
%
Figure~\ref{fig:qt-tableview-basic} shows the resulting widget. We
could also pass our model to any other view expecting a
\class{QAbstractItemModel}. For example, the first column could be
displayed in a list or combo box.

The \R{} data frame of a \class{DataFrameModel} may be accessed using
\index{general}{qdataFrame}\function{qdataFrame}:
<<qt-mvc-qdfm-access>>=
df <- qdataFrame(model)
df[1:3, 1:10]
@ 
...
miles per gallon ratio. The \class{DataFrameModel} object is a
reference, so modifications occur in place, rather than being
incorporated in a newly constructed object. One consequence is that
changes made within a function body may propagate beyond the local
environment. It is important to notice that any view of the model will
reflect changes to the underlying model without any explicit updating.

\paragraph{Headers}
A table view has a horizontal and vertical header. The horizontal header
...
the model will be \iprogram{garbage collection}garbage collected, from lack of an owner. For
example, this does not work:
<<qt-mvc-table-gc,results=hide>>=
...
the model, like this:
<<qt-mvc-table-parent,results=hide>>=
...
inconsistent with the notation of \R. The conversion of the numeric
data to text is carried out by an \textit{item delegate}. Similar to a
\GTK{} cell renderer (Section~\ref{sec:RGtk2:mvc:GtkTreeView}), an
item delegate is responsible for the rendering and editing of items
(cells) in a view. Every type of item delegate is derived from the
\class{QAbstractItemDelegate} class. By default, views in \Qt{} will
use an instance of \class{QStyledItemDelegate}, which renders items
according to the current style. As \Qt{} is unaware of the notion and
encoding of missing values in \R, we need to give \Qt{} extra
guidance. The \pkg{qtbase} package provides the
\class{RTextFormattingDelegate} class for this purpose. To use it, one
creates an instance and sets it as the item delegate for the view:
<<qt-mvc-rtfd, results=hide>>=
...
programmer), although this depends on the resize mode. The
...
to width needed to fit contents). The \method{setResizeMode}{QHeaderView} method
changes the resize mode of a specific column.  Below, we make all of
our columns expand:
%% jv 7-29-11 this is a method, we had an assignment to a property
<<qt-mvc-table-stretch, results=hide>>=
...
list:
<<qt-mvc-lists-combo, results=hide>>=
...
<<qt-mvc-lists-sort, results=hide>>=
df <- qdataFrame(model)
qdataFrame(model) <- df[order(df$mpg),]
@ 
%

\paragraph{\class{QStringListModel}}
...
character vector:
<<qt-mvc-lists-qslm, results=hide>>=
model <- Qt$QStringListModel(rownames(mtcars))
list_view <- Qt$QListView()
list_view$setModel(model)
@ 
%
Now we can retrieve the values as a character vector with the
\method{stringList}{QStringListModel} method, rather than as a
data frame:
...
example, the items may be wrapped into additional columns or displayed
in an icon mode. The widget also supports unrestricted layout and drag
and drop.
...
lists and tables. 
There are five selection modes for item views, and they are defined by the
\qtenumeration{QAbstractItemView::SelectionMode}
enumeration, and include:
\\

\begin{description}
\item\qcode{SingleSelection} mode allows only a single item to
be selected at once.
\item \qcode{ExtendedSelection} mode, the default,
...
the selection.
\item \qcode{ContiguousSelection} mode disallows the
\kbd{Ctrl} key behavior. 
\item \qcode{MultiSelection} mode allows selection on mouse-over, with range
...
For our tabular view, selection may be row-wise, column-wise or item-wise
(\GTK{}, by comparison, supports only row-wise selection). By default, 
selection is by
...
\subsection{Accessing the Selection}
\label{sec:qt-access-selection}

The selection state is stored in its own data model,
\class{QItemSelectionModel}:
<<qt-mvc-selection-model>>=
selection_model <- list_view$selectionModel()
@
<<qt-mvc-selection-set-initial, echo=FALSE, results=hide>>=
...
<<qt-mvc-selection-multiple-set-initial-I, echo=FALSE, results=hide>>=
...
we retrieve that selection from the selection model:
...
\subsection{Responding to Selection Changes}
...
qconnect(selection_model, "selectionChanged", selectionChangedHandler)
@ 
% 
...
<<qt-mvc-selection-multiple-set-initial, results=hide>>=
...
the \class{QItemSelection} object by repeatedly calling its
...
@ 
% 
Querying a selection in a table view is essentially the same as for
the list view, except we can request indices representing entire rows
or columns. In this example, we are interested in the rows, where the
user has selected:
...
selection_model$select(selection, Qt$QItemSelectionModel$Select | 
...
\class{QSortFilterProxyModel}.\footnote{In \pkg{RGtk2} there is a
  separate proxy model for each of sorting and filtering,
  cf. Section\ref{sec:RGtk2:mvc:proxies}.}  After constructing an
instance and specifying the child model, the proxy model may be handed
to a view like any other model:

<<setModel, echo=FALSE, results=hide>>=
...
roles listed in the \qtenumeration{Qt::ItemDataRole}
enumeration, selectively listed in  Table~\ref{tab:qt:enum:itemDataRole}.
...
\paragraph{\class{DataFrameModel} roles}
\class{DataFrameModel} instances support role-specific values for each
item, provided \qcode{useRoles = TRUE} is passed to the
constructor. It is then up to the programmer to indicate the mapping
from a data frame column to a column and role in the model. The
mapping is encoded in the column names. Each column name should have
the syntax \qcode{[.NAME][.ROLE]}, where \qcode{NAME} indicates the
column name in the model\footnote{\qcode{NAME} can refer to multiple
  columns, if separated by periods, or all columns if omitted.} and
...
For example, to customize the rownames, we could shade the background of the first column, the
...
frame.\footnote{Storing objects other than atomic vectors in a data
  frame requires some care, which we avoid here. If we had added that
  column in a call to \qcode{data.frame} or \function{cbind}, it would
  have been necessary to wrap the list with \code{I()} in order to
  prevent coercion of the list to a data frame.}
...
the documentation for that class.  Due to implicit conversion in the
...
After these modifications, the model can be passed to a view, as in
Figure~\ref{fig:qt-dataframemodel-role-example}.
<<showView, results=hide>>=
view <- Qt$QTableView()
view$setModel(model)
...
\begin{figure}
  \centering
  \includegraphics[width=.8\textwidth]{fig-qt-dataframemodel-role-example}
  \caption{Example decorating cell items using role specification of \class{DataFrameModel}}
  \label{fig:qt-dataframemodel-role-example}
\end{figure}

If the \qcode{NAME} component is omitted, the role will apply to all
columns for which a role of the same type has not already been
specified. Here, we change change the foreground color for all cells:
<<qt-mvc-role-all-font, eval=FALSE>>=
qdataFrame(model)$.foreground <- list(qcolor("darkgray"))
@ 


\paragraph{Roles in other models}
...
argument to the model's \method{setData}{QAbstractItemModel}
method. The value of \argument{role}{setData} defaults to
\qcode{EditRole}, meaning that the data is in an editable form, i.e.,
it is treated as input from the user. 

Here, we show how to create a list view and set the background of the
first item to yellow:
...
\Qt.  Hierarchical data in \R{} often arises when splitting a tabular dataset
...
<<qt-mvc-standard-item-set-item, results=hide>>=
...
\begin{figure}
  ...
  \begin{minipage}[c]{.3\textwidth}
    \includegraphics[width=\textwidth]{fig-qt-treemodel-treeview}
  \end{minipage}\quad
  \begin{minipage}[c]{.3\textwidth}
    \includegraphics[width=\textwidth]{fig-qt-treemodel-tableview}
  \end{minipage}\quad
  \begin{minipage}[c]{.3\textwidth}
    \includegraphics[width=\textwidth]{fig-qt-treemodel-listview}
  \end{minipage}
  \caption{The \code{tree_model} instance viewed in a tree view, a table view and a list view}
  \label{fig:qt-treemodel-views}
\end{figure}

As before, we create a \class{QModelIndex} object for
accessing each cell of the model (in line 3). We need to add rows and columns to
each manufacturer node before creating its children (lines 5 and 6). This nested loop
approach to populating a model is much less efficient than converting
a \class{data.frame} to a \class{DataFrameModel}, but here is necessary
to communicate the hierarchical information.

\paragraph{The \class{QStandardItem} class}
...
for populating the model simplifies to:
<<echo=FALSE>>=
tree_model <- Qt$QStandardItemModel(rows = 0, columns = 1)
@ 
...
by(Cars93, Cars93$Manufacturer, function(df) {
  ...
  manufacturer_item <- Qt$QStandardItem(man)
...
The \class{QTreeView} widget displays the data in a table, with the
...
workspace browser example, below.
...
%
Figure~\ref{fig:qt-treemodel-views} shows the \code{tree_model} in
the three separate types of views we've discussed, the left most being
with in a \class{QTreeView} instance, as just illustrated.
...
included in an analysis. In the view, the user will be able to use a
combo box to choose between \code{TRUE} and \code{FALSE}. We could
display an editable \code{Analyze} column by adding a column named
\code{.Analyze.edit}, but instead we take advantage of a convenience
of \class{DataFrameModel}. We simply add the \code{Analyze} column and
pass its name as the \code{editable} argument to \function{qdataFrameModel}:
<<qt-mvc-edit-analyze>>=
df <- mtcars 
df$Analyze <- TRUE
...
\class{QListWidget} is an easy-to-use widget for displaying a set of
items for selection. (Figure~\ref{fig:qt-listwidget-styles}.)  As with
combo boxes, we can populate the items directly from a character
vector through the \method{addItems}{QListWidget} method:
...
specified, the item may be added through the methods
...
<<qt-mvc-listwidget-selectionchanged, results=hide>>=
...
\begin{figure}
  ...
  \begin{minipage}[c]{.45\textwidth}
    \includegraphics[width=\textwidth]{fig-qt-listwidget-selection}
  \end{minipage}\quad
  \begin{minipage}[c]{.45\textwidth}
        \includegraphics[width=\textwidth]{fig-qt-listwidget-checkbutton}
  \end{minipage}
  \caption{Two easily implemented styles for selecting items from a
    \class{QListWidget} instance: the traditional selection and using
    checkbuttons.}
  \label{fig:qt-listwidget-styles}
\end{figure}


\paragraph{Using check buttons for selection}
It is often easier for the user to select multiple items by clicking a
check button next to the desired items. The right figure in
Figure~\ref{fig:qt-listwidget-styles} shows an example. The check box is only shown if
...
we set all of the items to unchecked to show the check buttons,
check the selected items, then turn off selection.
...
@ 
%
To get the selected items, one can \iprogram{iterator}iterate over the items, as above,
and invoke the \method{checkedState}{QListWidgetItem} method:
<<>>=
...
is likely preferable to use \class{QListView}, \class{DataFrameModel} and
...
The basic interface of a model requires that at a minimum the methods
\meth{rowCount}, \meth{columnCount}, and \meth{data} be provided.
The first two describe the size of the table for any views. We have
already demonstrated the use of the \meth{data} method in the previous
sections, it provides data to the view for a particular cell
\textit{and} role. For example, if one is displaying numeric data, the
...
For editable models, one must also implement the \meth{flags} method to
...
\section{Implementing custom views}
...
\begin{example}{Mapping selected model items to a text entry}{ex-qt-mapping-model}
  \SweaveInput{ex-qt-mapping-model}
\end{example}




Now, let us consider a different problem: summarizing or aggregating
...
widget, which is the view and controller for a \class{QTextDocument}
model. The model may be shared amongst many different views, allowing
for synchronized buffers.

\class{QTextEdit} supports both plain and rich text in HTML format,
including images, lists and tables. Applications that display only
plain text may be better served by \class{QPlainTextEdit}, which is
faster due to a simpler layout algorithm. \class{QPlainTextEdit} is
...
Here, we create a \class{QTextEdit} instance and populate it with some
text. Although the text is actually stored in a \class{QTextDocument} instance,
...
## not shown
edit$show(); edit$raise()
@ 

The underlying \class{QTextDocument} instance can be set by the
\method{setDocument}{QTextEdit} method, but need not be, as one is
created on construction.

Adding text to the document can be done easily through the slots
\method{setPlainText}{QTextEdit}, which replaces the existing text, or
\method{append}{QTextEdit} which appends the text as a new paragraph
...
As described in its manual page, the widget works on paragraphs and
characters, a paragraph being a formatted string, word-wrapped to fit
into the width of the widget. For plain text, new lines signify
paragraphs.

To return the contents of the model as text, the
\method{toPlainText}{QTextEdit} method is available:

<<>>=
text_edit$toPlainText()
@ 
%
The hard line break \backslashn{} is present, as \meth{append} created
a new paragraph.

%% undo/redo
When text is added to a buffer, it can be undone through the
\method{undo}{QTextEdit} slot. There is also \code{redo} to reverse
the decision and \code{undoAvailable} and \code{redoAvailable} to check
for the possibility of each action.

\paragraph{HTML support}
Instead of plain text, one can also add and insert HTML formatted text
for display. The slots \method{setHTML}{QTextEdit} and
\method{append}{QTextEdit} may be used. The
\method{toPlainText}{QTextEdit} method will return the text with
mark-up stripped off, whereas \method{toHtml}{QTextEdit} will return
...
interact with a text cursor object, of class
\class{QTextCursor}. Here, we obtain the user-visible cursor and move
...
\class{QTextEdit} through its \method{setTextCursor}{QTextEdit} method. This behavior is often convenient, because it
allows us to modify arbitrary parts of the document, without affecting
the user cursor.  For example, we could insert a 32 by 32 pixel image at the beginning:
%% JV 7-29-11 added qsize call to actualSize to get out of erro
...
In the above we moved the cursor through its
\method{setPosition}{QTextCursor} method. If the document is viewed as
a single string of characters, the position $i$ would refer to the
space between the $i$th and $i+1$st character, $0$ being the initial
point in the document.

The motion of the
cursor is described by the \qtenumeration{QTextCursor\$MoveOperation}
enumeration, with several values such as \qcode{Start}, \qcode{End},
\qcode{StartOfLine}, \qcode{EndOfLine}, \qcode{StartOfWord},
\qcode{EndOfWord} etc. 

For example, to move the cursor to the start of
the second line, we could do:
<<results=hide>>=
cursor <- text_edit$textCursor()
...
@ 
%
The \code{NULL} value indicates that the user has not selected any
text.  

A text cursor has an anchor position, in addition to its position. The
\igui{text selection}selection is the text between the two. When moving
the cursor through its \method{movePosition}{QTextCursor} method, one
can choose to move or keep the anchor in place.  
Normally, the anchor and cursor are at the same position. To
make a selection programatically, we move the cursor independently of
its anchor. The \qtenumeration{QTextCursor\$MoveMode} enumeration with
values \qcode{MoveAnchor} and \qcode{KeepAnchor} may be specified to
\meth{movePostion} to control this. Here we set the selection to
include the first three words of the text in the second line, we have:
<<results=hide>>=
...
@
%%
The $3$ specified to \meth{movePostion} calls the action $3$ times.
...
There are several different signals emitted by \class{QTextEdit}
instances: \signal{textChanged}, when the text changes;
\signal{cursorPositionChanged}, when the cursor position changes; and
\signal{selectionChanged} when the selection changes (according to the user
visible cursor). For the latter, the \signal{copyAvailable}
signal is largely equivalent, except it passes a boolean argument
indicating whether the selection is non-empty.
...
editor, this is not desirable. The \property{lineWrapMode}{QTextEdit} property
...
@
% 
One can fix the wrapping at a certain number of characters by using a
wrap mode of \code{FixedColumnWidth} and setting the count through
\method{lineWrapColumnOrWidth}{QTextEdit}.
...
\paragraph{Character attributes}
The widget keeps track of a current set of formatting options in an
object of class \class{QTextCharFormat}. The text edit methods
\method{setCurrentFont}{QTextEdit}, \method{setFontFamily}{QTextEdit},
and \method{setFontWeight}{QTextEdit}, among others, modify the
current settings. If called when there is a selection, the change will
be applied to the selection in addition to any new text.
...
\begin{figure}
  \centering
  \includegraphics[width=.5\textwidth]{fig-qt-textedit-view-context-menu}
  \caption{Context menu showing completion candidates for the token
    \qcode{mean.} taken from the current selection.}
  \label{fig:qt-textedit-view-context-menu}
\end{figure}
...
As we introduce in Section~\ref{sec:qt:app:context-menus} of
Chapter~\ref{cha:qt:MVC}, one can enable a dynamic context menu on a
widget by overriding the \meth{contextMenuEvent} virtual.  For our
...
action for every possible completion
(Figure~\ref{fig:qt-textedit-view-context-menu}). Triggering an action
will paste the completion into the document replacing the current
selection with the chosen completion candidate.
...
%% JV 8-3-11 Removed this, it seemed to long for what it offered
