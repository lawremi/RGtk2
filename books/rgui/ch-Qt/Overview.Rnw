...
(\acronym{MOC}). The MOC allows for convenient syntax in the
...
the the properties and methods of the instance. For \code{w}, we list
...
@ 
%
We have passed three different types of object as the
first argument to \function{Qt\$QPushButton}: a \class{QWidget}, a
string, and finally a \class{QIcon}. The dispatch depends on only on
the type of argument, unlike the constructors in \pkg{RGtk2}, which
dispatch based on which arguments are specified.
...
signal handler. The user data serves to parameterize the callback.
...
bottom, etc. \Qt{} defines enumerations within the relevant class or,
...
Although these enumerations can be specifed as integers, they are
...
alignment style. For example, aligning text in label in the upper
...
@ 
%
To check if the alignment is to the right, we could write:
<<>>=
as.logical(flag & Qt$Qt$AlignRight)
@ 

\section{Defining Classes and Methods}
\label{sec:defin-class-meth}

As \Qt{} is a implemented in an object-oriented language, C++, the
...
introductory example \XXX{reference} we saw this minimal use of
