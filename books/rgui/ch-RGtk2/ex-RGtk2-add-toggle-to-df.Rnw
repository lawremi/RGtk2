...
old_packages <- old.packages()[,c("Package", "Installed", "ReposVer")]
...
<<>>=
doUpdate <- function(old_packages) install.packages(old_packages$Package)
@ 
...
view$insertColumnWithAttributes(0, "", cell_renderer, active = ncol(d))
...
gSignalConnect(button, "clicked", function(button, data) {
  view <- data
  model <- view$getModel()
  old_packages <- model[model[, ncol(model)], -ncol(model), drop = FALSE]
