...
alpha-blending and antialiasing. It reports user events through the
\function{getGraphicsEvent} function.
...
context. The off-screen drawing could then be composited with other
images when displayed. Example~\ref{ex:RGtk2:pixbuf} generates an icon
by pointing the device to pixmap. Finally, passing a \class{GtkPrintContext} to
...
%
  ...
  directly, print through a dialog, show a print preview or export
  directly to a file. Before any such actions can be performed, we need
  to implement the rendering of our document into printed form. This is
  accomplished by connecting to the
  \signal{draw-page}{GtkPrintOperation} signal. The handler is passed a
  \class{GtkPrintContext}, which contains the target Cairo context. In
  general, one would call Cairo functions to render the document,
  which is beyond our scope. In this case, though, we can pass
  the context directly to \pkg{cairoDevice} for rendering the R plot:
<<gtk-cairo-draw-page>>=
...
<<gtk-cairo-run-dialog>>=
printOp$run(action = "print-dialog", parent = NULL)
@
% 
  When the user confirms the dialog, the
  \signal{draw-page}{GtkPrintOperation} handler is invoked, and the
  rendering is sent to the printer.
\end{example}
