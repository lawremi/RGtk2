...
%% FIXME: reworked this example; probably completely broken now
...
We subclass \class{QMainWindow} so that we can add a toolbar. Our
constructor customizes the notebook, sets up the actions and toolbar,
then opens with a blank sheet:
<<results=hide>>=
...
@ 
%
Our GUI might also benefit from a menu bar, an exercise left
for the reader.
