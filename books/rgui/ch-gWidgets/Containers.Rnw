...
method will initiate this recompuation. This may be necessary to get the
...
\item[expand, fill] The underlying layout algorithms have a means to
  ...
  expands. Generally, those widgets which have \code{expand=TRUE}
  specified will get the excess space shared amongst
  them. 
...
\item[fill, anchor] When a child widget is placed into its allocated
  ...
  allocation (only when \code{expand=TRUE}).~\footnote{For \GTK, filling always
  occurs orthogonally to the direction of packing.}
...
\begin{figure}
  ...
  \caption{
    ...
    middle and \pkt{gWidgetsQt} on right.). For \GTK\/ child
...
l <- gbutton("expand=FALSE", expand=FALSE, cont=g)
gseparator(cont=g)
l <- gbutton("expand=TRUE, fill='x'", expand=TRUE, fill="x", cont=g)
gseparator(cont=g)
l <- gbutton("expand=TRUE, fill='y'", expand=TRUE, fill="y", cont=g)
gseparator(cont=g)
l <- gbutton("expand=FALSE, anchor=c(1,0)",anchor=c(1,0), cont=g)
update(w)
@ 
...
disclose or hide some additional information. (How each toolkit
resizes when the widget collapse varies.)  This class essentially
subclasses \code{gframe} where the
...
widgets in the above example can be done through:
<<main_table_prop>>=
vals <- sapply(seq_len(nrow(tbl)), function(i) {
  svalue(tbl[i,2])
})
@ 
