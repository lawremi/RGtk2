...
model:
<<rgtk2-mvc-insert-column-hardway, print=FALSE>>=
vc <- gtkTreeViewColumn()
vc$setTitle("Manufacturer")
cr <- gtkCellRendererText()
vc$packStart(cr)
vc$addAttribute(cr, "text", 0)
vc$insertColumn(vc, 0)
@ 
%
A column with
...
<<rgtk2-mvc-insert-column-easyway>>=
...
<<rgtk2-mvc-insert-all-columns>>=
view <- gtkTreeView(model)
mapply(gtkTreeViewInsertColumnWithAttributes, -1, colnames(model), 
...
<<echo=FALSE>>=
vc <- gtkTreeViewColumn()
view$insertColumnWithAttributes(vc, 0, gtkCellRendererText(), text = 0)
## pack in GUI
...
row indices, like that returned by \Rfunction{order}.
...
%% TODO: simple example right here
The underlying
