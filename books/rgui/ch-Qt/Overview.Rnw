...
(\url{http://github.com/ggobi/qtbase}) and is distributed by
Bioconductor (\url{http://www.bioconductor.org/}).  It depends on the
\Qt{} framework, available as a binary install from
...
\Qt{} uses an architecture of signals and slots to have
...
qconnect(button, "clicked(bool)", function(checked) print(checked))
@ 
...
proxy <- qconnect(button, "clicked", function() message("ouch"))
...
%
%% JV did this change???
However, the property is not actually exposed by \Qt\/ to dynamic
systems, like the \pkg{QtScript} Javascript engine, which would only
understand \Qt\/ types. To export a property, one must provide the
...
in the object, named by the format \qcode{.property}. One can override
the default behavior by passing a getter and/or setter function to the 
\argument{read}{qsetProperty} and/or \argument{write}{qsetProperty}
...
\property{whatsThis}{QWidget} property in response to a query, such as
...
button$show()
button$styleSheet <- "QPushButton {color: red; background: white}"
@ 
...
stylesheet to every widget matching \code{what} in the
application. The default for \code{what} is \code{"*"}, meaning that
the stylesheet applies to any widget class. The following would cause
all widgets in the application to have the same colors as the button:
