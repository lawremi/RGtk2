...
%% FIXME: these need to be ported to QTableView
...
loss in flexibility from relying on a hard-coded data
model. \class{QTableWidget}, in particular, precludes the use of
...
\class{QListWidget}, out of an acknowledgement that displaying a
short simple list of items is a common task in a GUI.
