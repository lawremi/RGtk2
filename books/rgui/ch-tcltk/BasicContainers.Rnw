...
preferred width and height and to specify a menubar specified through
...
\function{tkwm.deiconify} function. Either of these can be useful
in the construction of complicated GUIs, as the drawing of the
...
is specified as a string, as was described for \function{tkwinfo}.
...
For dialogs, a top-level window should be related to
...
then it need only be specified once for the top-level window. For
example, one can define keyboard accelerators this way.
...
\begin{example}{A window constructor}{ex-tcltk-window}
  ...
  allowing some useful options to be passed in.
<<newWindow>>=
...
These commands are illlustrated below, where we show how to rotate around the
child components in a frame,
<<rotateButtons>>=
w <- tktoplevel()
f <- ttkframe(w)
tkpack(f)
# pack in button
for(i in 1:3) {
  b <- ttkbutton(f, text=as.character(i))
  tkpack(b, side="left")
}
# rotate leftmost to rightmost
rotate <- function() {
  children <- as.character(tkpack("slaves", f))
  tkpack.forget(children[1])
  tkpack(children[1], after=children[3], side="left")
}
@ 
...
the extra space allocated in an even manner to it. The
