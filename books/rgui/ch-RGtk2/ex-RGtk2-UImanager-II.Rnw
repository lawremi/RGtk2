...
\meth{getWidget}{GtkUIManager} method. The following code sketches
...
pattern is used to \iprogram{encapsulation}encapsulate a request
(method call) as an object. A basic command object has just one
method, \meth{execute}. Any needed parameters are stored in the object
as properties. The command pattern has GUI-related applications beyond
the undo/redo stack, including the action objects (i.e., instances of
\class{GtkAction}) that are managed by \class{GtkUIManager}.
