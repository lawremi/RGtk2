...
## The following uses \code{addBinding} to toggle the checkbutton value when the \kbd{return}
## key is pressed when the widget has the focus.
...
\begin{example}{Using validation for dates}{ex:tcltk-validation-dates}
  \SweaveInput{ex-tcltk-validation.Rnw}
\end{example}
