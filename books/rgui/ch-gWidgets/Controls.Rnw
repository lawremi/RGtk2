...
In \pkg{gWidgets} the abstract view for selecction widgets is that the
...
\begin{example}{Updating comboboxes}
  A common pattern in many GUIs is to have one combobox update another
  once a selection is made. The following shows how this can be
  done. First we set up the GUI:
<<>>=
w <- gwindow(visible=FALSE)
tbl <- glayout(cont=w)
#
tbl[1,1] <- "Manufacturer"
tbl[1,2] <- (man <- gcombobox(sort(unique(Cars93$Manufacturer)), cont=tbl))
tbl[2,1] <- "Model"
...
addHandlerChanged(man, handler=function(h,...) {
  poss <- subset(Cars93, subset=Manufacturer == svalue(man), select="Model", drop=TRUE)
...
availDfs <- function(envir=.GlobalEnv) {
  x <- ls(envir=envir)
  ind <- sapply(mget(x, envir), is.data.frame)
  data.frame(dfs=x[ind], stringsAsFactors=FALSE)
}
#
