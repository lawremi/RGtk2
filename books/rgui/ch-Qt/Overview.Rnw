...
%% Could pack the label into a layout on the QLineEdit, and then use
%% the stylesheet setting to prevent text overwrite of it.
...
n\begin{example}{A ``error label''}{eg:qt-error}
...
This example shows how to extend the line edit widget to display an
error state. Such a widget might prove useful when one is validating
the entries values.
...
image to the left of the entry message and set the tooltip. The use of
\code{base:::paste}, as opposed to simply \code{paste}, is necessary
here as the \class{QLineEdit} class has a slot called \code{paste}
(for copy-and-paste activities). As the search list gets modified to
place the objects environment in front, we must reference
\function{paste} explicitly.
<<>>=
qsetMethod("setError", LineEditWithError, function(msg) {
  f <- system.file("images/cancel.gif", package="gWidgets")
  ss <- base:::paste("* {",
              base:::paste(sprintf("background-image:url(%s)", f),
...
Clearing the error is s simple matter of resetting the properties to \code{NULL}.
<<results=hide>>=
...
%% JV: Definitely for the size issues. But shouldn't some common
%% methods be separated out (I'm thinking along the lines of inherited
%% QWidget methods that are common to many widget)
