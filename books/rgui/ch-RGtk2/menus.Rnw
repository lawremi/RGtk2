...
% MOVEME: GtkAction is essentially a data model, with menubar and
% toolbar as views. All of this could be moved to widgets with models chapter.
...
toolbars. The \constructor{gtkAction} constructor creates actions,
taking arguments \argument{name}{gtkAction},
...
Menubars (along with the upcoming toolbars and statusbars) are placed
...
display them, any icon specified through the action's \code{stock.id}
...
\code{SetActive(TRUE)}. When the user clicks on the menu item, its
...
%% MOVEME: status bars can be described in the basic controls section
\section{Statusbars}
