...
There is no native calendar widget in \pkg{tcltk}. This example shows how one can use the validation framework for entry wdgets to check that user-entered dates conform to an expected format. First, we define some patterns for acceptable date formats.
<<>>=
datePatterns <- c()
...
For each, a number of substition values are possible, in addition to
