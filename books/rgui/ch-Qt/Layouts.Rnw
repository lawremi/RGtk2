...
In this chapter we discuss how to program GUI layouts. 
...
parameters. Every widget returns a size hint to the layout from the
...
size policies are:
\begin{description}
\item[Fixed] to require the size hint exactly,
\item[Minimum] to treat the size hint as the minimum, allowing expansion,
\item[Maximum] to treat the size hint as the maximum, allowing shrinkage,
\item[Preferred] to request the size hint, but allow for either
  expansion or shrinkage,
\item[Expanding] to treat like \code{Preferred}, except the widget desires 
  as much space as possible,
\item[MinimumExpanding] to treat like \code{Minimum}, except the widget desires 
  as much space as possible,
\item[Ignored] to ignore the size hint and request as much space as possible.
\end{description}
...
depending on the label dimensions and has the policies \code{Minimum}
and \code{Fixed} as its horizontal and vertical policies,
