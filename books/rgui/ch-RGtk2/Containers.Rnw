...
indexing. This code will move the last child to the second position:
<<>>=
b3 <- g[[3]]
...
this depends on the \property{tab-pos} property, with a value from
\code{GtkPositionType}: \qcode{left}, \qcode{right}, \qcode{top}, or
\qcode{bottom}. By default, the tabs are on top. We move them to the
bottom:
...
\property{page} property holds the zero-based index of the active
