...
@ 
%
The \qcode{QStyle::StandardPixmap} enumeration lists all of the
...
device and plot the icon with \pkg{grid}:
<<echo=FALSE, results=hide>>=
require(grid)
require(ggplot2)
@ 
...
By attaching a callback to the \signal{buttonClicked}
signal~\footnote{See Section~\ref{sec:qt-signals} for why we need the \code{(QAbstractButton)}.}, we will
be informed when any of the buttons in the group are clicked:
<<results=hide>>=
qconnect(bg, "buttonClicked(QAbstractButton*)", function(button) {
...
The simplest way to group the radio boxes is to place them into the
same layout:
...
process. The signals \signal{loadStarted}{QWebView},
\signal{loadProgress}{QWebView} and \signal{loadFinished}{QWebView}
...
widgets into a web page. This is one mechanism for constructing hybrid
web/desktop applications. Widget embedding is implemented through the
standard HTML \qcode{object} tag. We can register a plugin, manifested
as a \class{QWidget}, for a particular mime type, specified through
the \qcode{type} attribute of the \qcode{object} element.
...
For our plugin, we use the class browser widget, constructed above. To
provide the plugin, we need to implement a custom
\class{QWebPluginFactory}:
<<qt-widget-webview-plugin-factory>>=
...
plugin descriptions:
<<qt-widget-webview-plugin-factory-plugins>>=
...
@ 
%
Our factory provides a single plugin, with a single mimetype that
...
<<qt-widget-webview-plugin-factory-create>>=
...
<<qt-widget-webview-plugin-factory-register>>=
