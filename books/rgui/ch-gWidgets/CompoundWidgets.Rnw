...
following is a simplemodification the example from the help page for \code{tkrplot}:
...
bb<-1
img <-tkrplot(getToolkitWidget(g), function() plot(1:20,(1:20)^bb))
tkpack(img)                             # add widget
f<-function(...) {
  b <- svalue(sl)
  print(b)
  if (b != bb) {
...
sl <- gslider(from=0.05, to=2, by=0.05, cont=g, handler=f, expand=TRUE)
visible(w) <- TRUE
