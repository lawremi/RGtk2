...
class Parameter(object):
    def __init__(self, ptype, pname, paccess, pdflt, pnull, prop=None):
        self.ptype = ptype
        self.pname = pname
	self.paccess = paccess
        self.pdflt = pdflt
        self.pnull = pnull
        
    def __len__(self): return 5
    def __getitem__(self, i):
        return (self.ptype, self.pname, self.paccess, self.pdflt, self.pnull)[i]
	...
	if old.paccess is not None:
	    self.paccess = old.paccess
...
class MethodDefBase(Definition):
	...
	for arg in get_valid_scheme_definitions(args):
		...
		self.ret = arg[1]
		    ...
		    paccess = 'in'
			    ...
			    paccess = 'in-out'
                    self.params.append(Parameter(ptype, pname, paccess, pdflt, pnull))
            elif arg[0] == 'varargs':
	...
	if self.deprecated:
        ...
        if self.params:
            fp.write('  (parameters\n')
            for ptype, pname, paccess, pdflt, pnull in self.params:
                fp.write('    \'("' + ptype + '" "' + pname +'"')
                if paccess: fp.write(' (' + paccess + ')')
        ...
        if self.params:
		    ...
		    if pdflt: fp.write(' (default "' + pdflt + '")')
                ...
                for prop in self.params:
                    ...
                    if prop.argname: fp.write(' (argname "' + prop.argname + '")')
                    fp.write(')\n')
                fp.write('  )\n')
            else:
