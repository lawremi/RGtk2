%% JV commented out setData calls -- why aren't they working? Seem to
%% match API?
...
<<qt-mvc-selection-select-rows, results=hide>>=
selModel$select(sel, Qt$QItemSelectionModel$Select | 
...
to display values from a model as text in a label. The base class for
a view of an item model is \class{QAbstractItemView}. This class has
many virtual methods, which may be overridden to custom the handling
of key presses, mouse actions, scrolling, selection, etc. Here we focus
on just enough for our example.
