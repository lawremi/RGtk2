...
widgets. In \pkg{tcltk} more work is involved, than with \pkg{RGtk2}
and \pkg{qtbase}. Steps are needed to make one widget a drop source, and other
steps are needed to make the other widget a drop target. 
...
the cursor is over. The motion binding is on the drag widget, but the
widget passed to the call back is the one the motion is over. We use
\function{tkwinfo} to find the widget containing the $x,y$-coordinates
of the cursor position.  We then generate the
\code{\VirtualEvent{DragOver}} virtual event for this widget, and if
this widget is different from the previous last widget, we generate
the \code{\VirtualEvent{DragLeave}} virtual event.
