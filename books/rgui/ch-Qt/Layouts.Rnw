...
This chapter will introduce the available layout managers. Widgets
that function primarily as containers, such as the notebook, are
...
right (top to bottom for a vertical box).  In addition to adding child
widgets, one can nest child layouts by calling
\method{addLayout}{QLayout}.
...
layout. Although the widget is no longer managed by a layout, its
parent widget is unchanged. The widget will not be destroyed (removed
from memory) as long as it has a parent. Thus, to destroy a widget,
one should set the parent of the widget \code{NULL} using
...
when they expand.  The stretch factor may be specified as an optional
parameter to \meth{addWidget} or modified later with
...
%% alignment
By default, all widgets attempt to fill the allocated space; however,
it is not always possible to achieve this effectively, as in the case
of labels.  The position of the widget within the space may be
specified by the \argument{alignment}{addWidget} argument to
\method{addWidget}{QBoxLayout}.
...
expanding. The fixed spacing between any two children is globally
controlled through the \method{setSpacing}{QBoxLayout} method. For
example, we could require 5 pixels between the buttons in our layout:
<<qt-layout-box-spacing>>=
g$setSpacing(5)
@ 
%
To add a fixed amount of space between two specific children, call the
\meth{addSpacing} method while populating the container.

An expanding, spring-like spacer between two widgets is known as a
\textit{stretch}. We add a stretch with a factor of $2.0$ and add a
child button that will be pressed against the right side of the box:
<<qt-layout-box-stretch>>=
g$addStretch(2)
...
perpendicular direction. For a horizontal box, this is the
height. We could achieve this by adding a widget with its maximum
height requirement set to the desired maximum height for the entire
layout. As a convenience, the box layout provides the \textit{strut}
for this purpose:
<<qt-layout-box-strut>>=
g$addStrut(10)
@ 
...
(FIgure~\ref{fig:qt-layout-example}). We create the window with the
grid layout:
<<results=hide>>=
w <- Qt$QWidget()
...
to the \code{stretch} factors. A stretch factor is set by calling the
\method{setColumnStretch}{QGridLayout} method.

%% spacing
The spacing between widgets can be set in both directions with
\method{setSpacing}{QGridLayout}, or set for a particular direction
with \method{setHorizontalSpacing}{QGridLayout} or
