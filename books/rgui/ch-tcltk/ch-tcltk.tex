...
\TK~\footnote{
  ...
  instructive. A main use of \pkg{tcltk} is within the \pkg{Rcmdr}
...
it through the \pkg{tcltk} package using the following
...
\begin{Sinput}
 ...
 #
 handler = function() print(sprintf("Hello %s", tclvalue(txtVar)))
...
\class{tclObj} being the primary one (\class{tclVar} and \class{tkwin}
...
\paragraph{\pkg{tcltk} convenience functions}
...
widgets, and buttons; to illustrate the basic usage of \pkg{tcltk},
...
``\code{.}'' is created. (This is \code{.TkRoot}.) In the variable name \code{.label}, from
...
managers in Chapter~\ref{sec:tcltk:basic-containers} \code{tkpack} and \code{tkgrid}

The \code{tkack} command packs the widgets into the parent container
...
%% textvariables
For the button and label widgets in out example, their \code{text}
...
\function{tkwait.window} will make a toplevel window modal waiting until it is
destroyed. In the following we use \function{tkwait.variable}, which
waits for a change to variable, in this case \code{flag} defined
...
intial message; and \meth{show\_init\_msg} and \meth{hide\_init\_msg}
...
Our accessor methods, \meth{set\_text} and \meth{get\_text}, must work around a possible intial message.
\begin{Schunk}
\begin{Sinput}
 TtkEntry$methods(
...
scrollbars when not needed, although the \pkg{tcltk2} package has some
...
The return value is of class \class{tclObj}.  Coercion to character
...
with \code{tkdelete(txt, "1.0", "end")}.
...
(\code{enc\_frame}) to hold the pieces of our GUI.~\footnote{This
  means that \function{tkpack} needs to be used to manage this parent.
...
\function{tkgrid} layout manager which allows us to grow the top frame
...
the items \code{make\_gui} method. The \function{newFilterItem} call
...
last method is simply a \code{remove} method which calls back up into
...
\begin{Sinput}
 FilterItemNumeric$methods(
           get_value = function() {
             ...
             eval(parse(text=xpr), envir=list_ref$df, parent.frame())
           })
...
\begin{Sinput}
 FilterItemCharacter$methods(
           select_values_dialog=function() {
             ...
             tkpack(f, expand=TRUE, fill="both")
             tkpack(ttklabel(f, 
               text="Select your values by extending selection"))
             ## selection
