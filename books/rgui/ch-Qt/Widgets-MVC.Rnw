...
example, the items may be wrapped into additional columns or displayed in an icon
mode. The widget also supports unrestricted layout and drag and drop.
...
\begin{figure}
  ...
  \end{minipage}
  \caption{The \code{treeModel} instance viewed in a tree view, a table view and a list view}
...
view$setModel(treeModel); view$show(); view$raise()
##
view <- Qt$QTableView()
...
%
Figure~\ref{fig:qt-treemodel-views} shows the \code{treeModel} in
...
listWidget$selectionMode <- Qt$QListWidget$NoSelection
@ 
%
