...
in proprietary software, as well. We assume version 4.7.3 or later in
the follwing. Popular software developed with \Qt{} include the
communication application Skype and the KDE desktop for Linux. The
desktop version of RStudio cleverly uses \Qt's \class{QWebView} widget
to leverage \Qt's cross-platform availability to present a
feature-rich web application on the desktop.
...
The function \index{qmethods}\function{qmethods} will show the methods defined for a
...
properties for our button:\index{qproperties}
<<list-properties>>=
head(qproperties(b))
@
%
...
in \pkg{RGtk2}. The function \index{qconnect}\function{qconnect} establishes the
...
should be passed to qconnect. For example, there are two signatures
...
bottom, etc. These styles are \iprogram{enumerations}enumerated by
...
@ 
% 
Within the body of a constructor, the \index{super}\function{super} variable refers
...
through the \index{qsetmethod}\function{qsetMethod} function.  For example, accessors
...
refers to the instance.  There is also a \index{super}\function{super} function
...
class using the \index{qsetProperty}\function{qsetProperty} function. Here is the simplest
usage:
...
behavior by specifying a function for the getter and/or the setter:
...
\begin{example}{A model for workspace objects}{ex-qt-ws-objects}
...
the focus by tab-navigation or mouse clicks (although this behaviour
...
\begin{figure}
  ...
  \caption{Styling a widget with a style sheet can dramatiically alter its appearance}
...
\begin{example}{A ``error label''}{eg:qt-error}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=.6\textwidth]{fig-qt-line-edit-error}
