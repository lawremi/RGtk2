...
sapply(list(basicLabel, ellipsized, rightJustified, pangoLabel), 
       gtkBoxPackStart, expand = TRUE, fill = TRUE)
w$showAll()
@ 
\end{example}
...
%% signals
The \class{GtkEditable} interface supports three signals: \signal{changed} when
...
basic, text-only combo box is constructed by
