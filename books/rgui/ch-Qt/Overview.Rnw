...
\XXX{An overview of Qt, qtbase and its development}
...
\Qt, that contains the bulk of the functionality. The
components of this environment preserve the class structure from
\Qt. For example, the \code{QWidget} class being represented through
the component \code{Qt\$QWidget}.
...
These inherit from functions, as they act as constructors
...
Here, the \code{w} object is an environment and the properties and
methods for this instance of the \Qt{} class that are available from
within \R{} comprise its components. For \code{w} the first few listed
using \function{ls}:
<<eval=FALSE>>=
ls(w)
@ 
...
operator, but \verb+[[+ and \code{get} will also work. The
...
how it can be accessed and set.
<<windowTitle>>=
w$windowTitle
...
However, most properties in \Qt{} are accessed through getter and
...
property name written in lower camel case, the convention \Qt{} uses
...
The class name prefaced with a tilde is the destructor for a
widget. We can call this as follows:
...
more typically is called by \Qt{} recursively showing the children when a top-level window is drawn.
The method \method{close}{QWidget} will close the widget. 
...
A widget may be drawn with its own window, or embedded in a more
...
instance, \Qt's rectangle class. Rectangles are parameterized by a
point and two dimensions (\method{x}{QRect}, \method{y}{QRect},
\method{width}{QRect} and \method{height}{QRect}). In this case, the
point is the upper left coordinate and dimensions are in pixels. The 
...
specifying a minimum size or maximum size. The properties
\property{minimumSize}{QWidget}, \property{minimumWidth}{QWidget},
\property{minimumHeight}{QWidget}, \property{maximumSize}{QWidget}
\property{maximumWidth}{QWidget} and \property{maximumHeight}{QWidget}, and their
corresponding setters, are the germane ones.  One can also adjust
the \property{sizePolicy} property to fine-tune how widgets
expand. For example, buttons will only grow in the $x$ direction -- not the $y$
direction due to their default size policy.
...
top, bottom, etc. In \Qt{} these are represented by integer values whose value
can be gleaned from the appropriate manual page. However, \pkg{qtbase}
provides these values as named enumerations in the \code{Qt\$Qt} object. For example, right
alignment is specified with
<<Enumeration-example>>=
Qt$Qt$AlignRight
@ 

Thes values are used as follows, the last case showing how the
\verb+|+ operator is used to combine alignments.
<<alginexample, results=hide>>=
l <- Qt$QLabel("Our text")
l$setAlignment(Qt$Qt$AlignRight)
l$setAlignment(Qt$Qt$AlignRight | Qt$Qt$AlignVCenter)
@ 

A full list of the enumerated values can be teased out of the
...
receive. A palette has an enumeration of \code{ColorRole}s. From the
man page, these
...
\code{Qt\$QWidget} -- the function, not the call, so no parentheses); 
...
class is defined. Of these, the constructor is the most important. In
the introductory example \XXX{reference} we saw this minimal use of \function{qsetClass}.
<<results=hide>>=
...
\function{super} to inherit the parent's methods. 

Within the body of the constructor, the variable \code{this} is a
reference to the instance of the class and the inherited method names
are also attached, so need not be referenced through the \$\/ notation.


To define a method for a class the \function{qsetMethod} function is
used. The signature includes \code{name}, for the new name;
\code{class}, for the class being extended; and \code{FUN} to define
the method. The dispatch happens on the class and method name, not on
the signature of the method. In addition, there is an argument
...
the use of \function{super} below to call the next method.
<<results=hide>>=
