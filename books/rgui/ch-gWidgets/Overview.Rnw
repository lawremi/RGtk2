...
functionality with less technically proficient colleagues, etc. In many cases 
the
...
components do.\ilayout{widget hierarchy} In \pkg{gWidgets}, for the sake of portability, the
