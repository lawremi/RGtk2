...
\begin{figure}
  ...
  \caption{A GUI using dockable widgets and a \class{QMainWindow}
    instance}
...
@ 
%
We add a menu bar and toolbar. The actions are minimal, not
including icons, commands etc. We only show the file menu definitions.
