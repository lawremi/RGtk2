...
and an alignment enumeration.

How size gets allocated to the widgets managed by a layout depends on
many things. The \Qt{} documentation for layouts described this as
follows.  A widget has methods \method{sizeHint}{QWidget} to provide
the needed size for the widget. The default policy is that this is the
preferred size, but it can be resized. One can change the policy
through the \method{setSizePolicy}{QWidget} method. One sets the value
for the horizontal and vertical sizing. Values are taken from the
\qtenumeration{QSizePolicy} enumeration with possible values of
\qcode{Fixed} (\verb+Qt$QSizePolicy$Fixed+) for using the size hint;
\qcode{Minimum}, for size hint being minimal and sufficient;
\qcode{Maximum}, for the size hint being the maximal size;
\qcode{Preferred}; \qcode{Expanding}, for able to expand to fill the
space; \qcode{Ignored}, to ignore the size hint and give the widget
all the possible space. For exmaple, a button, by default, will
expand in the horizontal direction unless the size policy is
adjusted:
<<results=hide>>=
...
when they expand.
When more than one widget can expand, how the space gets allocated is
determined by their stretch factors (for box layouts set by the
\method{addWidget}{QBoxLayout} method). Widgets with a stretch factor of
$0$ get extra space only if no other widgets have a positive stretch factor. Of those, one
with an expanding size policy get allocated space first. 

If after this allocation, widgets haven't received their minimial size
request, then space is allocated to do so. Then is a widget is
allocated more than its maximal size, the allocation is shrunk.


%% alignment
When a widget is allocated additional space, the child can have its
alignment adjusted. The enumeration \qtenumeration{Alignment}
specifies values \qcode{Left} (\verb+Qt$Qt$AlignLeft+), \qcode{Right},
\qcode{HCenter} (horizontal center), \qcode{Justify}, \qcode{Top},
\qcode{Bottom}, \qcode{VCenter} (vertical center), and
\qcode{Center}. Values may be ``or''ed together.
...
the widget's parent to \code{NULL} using \method{setParent}{QWidget}.
...
\class{QScrollArea} class makes this very straightforward. The method
\method{setWidget}{QScrollArea} is used to add the child widget. To
ensure that a given widget is visible, the method
\method{ensureWidgetVisible}{QScrollArea} is available, where the
widget is passed as the argument.
...
related parts of GUI. Sometimes, just a separating line is all that is desired.
There is no separate separator widget in \Qt, unlike \GTK. However,
the \class{QFrame} class provides a general method for framed widgets
that can be used for this purpose with the appropriate settings.

The frame shape can be a box or others types but for this purpose a
line is desired. The enumerations \verb+Qt$QFrame$HLine+ or
\verb+Qt$QFrame$VLine+ can be passed to the method
\method{setFrameShape}{QFrame} to specify a horizontal or vertical
line. Its appearance can be altered by
\method{setFrameShadow}{QFrame}. A value of \verb+Qt$QFrame$Sunken+
or \verb+Qt$QFrame$Raised+ is suitable.
...
\class{QDialogButtonBox} \XXX{button reference}, which provides a container for buttons that
also tries to maintain an appearance consistent with the underlying
operating system.
...
(\method{setToolTip}{QAction}). The text and icon may be set at
...
shortcuts provided in the enumeration \verb+Qt$QtKeySequence+, for
example 
<<>>=
Qt$QKeySequence$Cut
@ 
...
column is selected) we disable the sub menu, otherwise we set the
...
package. We show how to offer these in a popup menu we do this for a button press (not the most
natural case):
...
method. The method \method{actions}{QWidget} will return a list of actions.
...
\verb+Qt$Qt$Horizontal+ or \verb+Qt$Qt$Vertical+. The toolbuttons can
...
the \qtenumeration{ToolButtonStyle} enumeration. The default is icon only, but
one could use, say, \verb+Qt$Qt$ToolButtonTextUnderIcon+.
...
can be adjusted through the \property{sizeGripEnabled} property.
...
temporary messagesl and \textit{permanent} where the message is never
hidden. In addition to messages, one can embed widgets into the statusbar.
...
desired widget through the dock widgets
\method{setWidget}{QDockWidget}. Widgets may go on any side of the
central widget. The position is specified through the
\qtenumeration{DockWidgetArea} enumeration, with values such as \verb+Qt$Qt$LeftDockWidgetArea+.
...
Floating a dock widget is intitiated by the user through clicking an icon in the
title bar or programatically through the
