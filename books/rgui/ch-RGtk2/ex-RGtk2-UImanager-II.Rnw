...
With this stack a typical action callback include something like this
one from the action to change a column name:
...
The data frame model (defined in our reference class \class{DfModel}) we define is a wrapper around an
