...
slider for adjusting the alpha level of the points. Usually, a slider
...
\class{GtkHScale} and overriding its \method{format_value}{GtkScale}
virtual method.
...
class overrides the \method{format_value} virtual from
...
\code{ma_data} object is a matrix of points that is meant to resemble
