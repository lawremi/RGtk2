...
For example, the \R\/ function \function{gtkContainerAdd} wrapped the
...
gives the constructors a somewhat consistent definition. For example,
<<gtkWindowNew>>=
gtkWindowNew
@ 
...
widget hierarchy. These include \method{GetParent}{gtkWidget} to get
the parent container; \method{GetToplevel}{gtkWidget} to get the top
widget in the objects widget hierarchy;
\method{GetAncestor}{gtkWidget} to search up the hierarchy for an
...
The \pkg{RGtk2} event loop integrates with the \R\/ event loop. Such
...
itself when using functionality such as \class{GtkExpander} and
...
specification. The interface is then built through a \class{GtkBuilder}
object.  The \code{glade} interface builder can be used to create the
