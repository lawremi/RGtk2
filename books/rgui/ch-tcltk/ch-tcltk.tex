...
\begin{Schunk}
\begin{Sinput}
 makeMonth <- function(parent, year, month) {
   ## add headers
...
\begin{Sinput}
 setMonth <- function() {
   ...
   cal_frame <<- ttkframe(frame)
   makeMonth(cal_frame, year, month)
...
\begin{Sinput}
 window <- tktoplevel()
 widget <- TtkEntry$new(parent = window, init_msg = "type value here")
 tkpack(widget$entry)
 #
...
\begin{Schunk}
\begin{Sinput}
 putLabel(label_frame, "alternative:", 3, 0)
...
\begin{Schunk}
\begin{Sinput}
 putLabel(label_frame, "conf.level:", 3, 2)
...
\begin{Schunk}
\begin{Sinput}
 putLabel(label_frame, "var.equal:", 4, 0)
...
display. A such, we define a function, \code{updateUI}, which does two
...
\begin{Sinput}
 updateUI <- function() {
   ...
   if(dfName == "") {
     ...
     tkconfigure(factor_combo, values = availFactors)
 ...
 updateUI()
 tkbind(data_combo, "<<ComboboxSelected>>", updateUI)
\end{Sinput}
\end{Schunk}
...
\begin{Sinput}
 if (!usingMac()) {
   tkbind(text_buffer, "<Button-3>", doPopup)
 } else {
   ...
   tkbind(text_buffer, "<Control-1>", doPopup)     # Control + click
 }
\end{Sinput}
\end{Schunk}
\end{example}
...
\begin{Sinput}
 ...
 tkconfigure(a$treeview, selectmode = "extended") # multiple selection
...
evaluate this within the data frame so that \code{value_variable} could use
variable names from the data framed. 
\begin{Schunk}
\begin{Sinput}
 FilterItemNumeric$methods(
       get_value = function() {
         xpr <- paste(nm, tclvalue(ineq_variable), tclvalue(value_variable))
         eval(parse(text = xpr), 
...
\begin{Sinput}
 FilterItemCharacter$methods(make_gui = function(parent) {
             ...
             sapply(list(label, in_label), tkpack,
                    side = "left", padx = 5)
             tkpack(button, expand = TRUE, fill = "x", side = "left")
             callSuper(parent)
           })
\end{Sinput}
\end{Schunk}
...
\begin{Sinput}
 insertChild <- function(treeview, node, parent = "") {
   l <- list(treeview, "insert", parent, "end", text = xmlName(node))
...
\begin{Sinput}
 ...
 scroll_frame <- scrollableFrame(frame, 300, 300)
\end{Sinput}
\end{Schunk}
