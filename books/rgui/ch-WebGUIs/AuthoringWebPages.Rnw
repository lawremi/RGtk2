...
\bottomrule
 \end{tabular}
  \caption{The \pkg{brew} delimiters and how they are processed. The plain
    \verb+<%+ processes within \R, \verb+<%=+ indicates to print the
    result, and \verb+<%#+ is a comment. A trailing \verb+-%>+
