...
reference so assignments can be made within function bodies. It is
important to notice that any view of the model will reflect changes to
the underlying model without needing additional programming.
...
inconstant with the notation of \R. The conversion of the numeric
...
\class{QSortFilterProxyModel}.~\footnote{In \pkg{RGtk2} one can only
  choose one or the other, cf. Section\ref{sec:RGtk2:mvc:proxies}.}
After constructing an instance and specifying the child model, the
proxy model may be handed to a view like any other model:
...
frame.~\footnote{Storing values directly into a data frame requires
  some care, which we avoid here. If we had added that column in a
  call to \qcode{data.frame} or \function{cbind}, it would have been
  necessary to wrap the list with \code{I()} in order to prevent
...
After these modifications, the model can be passed to a view, as in Figure~\ref{fig:qt-dataframemodel-role-example}.
...
columns in the model, as with this specification to change the font
size.

%% JV: 7-29-11 This is failing XXXXX
%% Error in map[headerInd] <- match(role, cn) : replacement has length zero
...
included in an analysis. We append \code{edit} to the column name, so
that the column performs the \code{edit} role. The user can then
toggle the \code{Analyze} values between \code{TRUE} and \code{FALSE}:

%% JV 8-3 XXX I can't get this to work!!
<<qt-mvc-edit-analyze, eval=FALSE>>=
df <- mtcars 
df$Analyze.edit <- TRUE
model <- qdataFrameModel(df, useRoles=TRUE)
@ 
...
\meth{rowCount}, \meth{columnCount}, and \meth{data} must be provided.
...
There are several different signals to connect to, emitted by
\class{QTextEdit} instances: the \signal{textChanged} signal when the text is changed;
the \signal{cursorPositionChanged} signal is emitted when the cursor
position changes; and the \signal{selectionChanged} signal is emitted
when the selection (according to the user visible
cursor) changes. For the latter, the \signal{copyAvailable} signal is largely equivalent, except it
...
The widget keeps track of a current set of formatting options a class
\class{QTextCharFormat}. The text edit methods
\method{setCurrentFont}{QTextEdit},
\method{setFontFamily}{QTextEdit}, and \method{setFontWeight}{QTextEdit},
among others, can be used to modify the current settings. If called
when there is a selection, the change will be applied to the selection
