...
@ 
%% not model$setDelegate(delegate)
%
...
#JV XXX model$setData(index, toupper(firstCar))
@ 
%
...
#JV XXXlistModel$setData(listModel$index(0, 0), "yellow", Qt$Qt$BackgroundRole)
listView <- Qt$QListView()
...
### JV XXXX the setData calls are failing here!
