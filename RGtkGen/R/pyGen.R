...
### NOTE: for simplicity, we force versions to be exactly equal
  ...
  for (name in paste(includes[!preparsed], "-", versions, ".gir", sep = "")) {
    paths <- c(name, file.path(includePaths, name))
    existing <- file.exists(paths)
    if (any(existing))
      db <- parseGIR(paths[existing][1], db)
    else stop("could not find GIR file for dependency: ", name)
  }
...
## Additional stuff we could use from GI:
## 
