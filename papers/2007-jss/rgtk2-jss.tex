...
the language.
%DTL:  This is a little dubious and simplistic.  Does it really help
%to try to divide users into separate camps.  I think the entire
%paragraph from here on could be replaced by something along the
%lines:
\begin{comment}% phrases as needed
  A general purpose programming language such as R requires
  non-trivial knowledge to use it to do anything. However, given a
  ...
  and restrictive, but typically makes performing the task for which
  easier.  It does this two different ways: a) stream-lining the steps
  involved in the task by providing a constrained context, and b) removing the
  need to remember function names and syntax.  Different users benefit
  from the two different interfaces for different tasks. And there is
  little doubt that for occasional users of a language and for 
  users focused a specific task, a GUI can greatly reduce the ease of
  getting started when compared to a general purpose programming language.
\end{comment} 
...
A more detailed and technical explanation of \pkg{GObject} is available in 
...
% an interface.  I don't think identifying the module as a machine helps.
...
The \pkg{RGtk2} API has also been designed for ease/efficiency of use. Towards this end,
...
By attaching an \proglang{R} handler function to a widget's signal, we can
...
%losing information about the object which is available from the property.
\begin{verbatim}
> window[["allocation"]]
$x
[1] 0
...
\pkg{GTK+} contains many types of layout containers besides boxes, including 
...
If the user answered ``Yes,'' the latest version of \pkg{RGtk2} will be 
...
to a frame.
%DTL: need to explain what a frame is and connect it to the frame widget.
...
return value from the \proglang{C} function. Finally, we retrieve the row index
...
%DTL:  The different color doesn't show up too well here, which is of course the
%point. But we should either say this or use a different value
%(e.g. .5) or use color.
...
%few pages.  It would be very valuable to provide a brief high-level overview of
...
%XML to express something that can be easily specified in R is bizarre.
...
\end{verbatim}
%DTL: Where is the rest of this function? It is very hard to read with
% the code and text intermingled in free form.
% Breaking the function into separate function calls with names that
% suggest what they are doing would be good.
%
%DTL: This paragraph is technically correct, but not very informative or 
...
key, we update the filter by the result of the \proglang{R}
expression.
...
%DTL: This ending } for the top-level function definition is hard to
...
\pkg{playwith} \citep{playwith}, a package for interactive \proglang{R}
