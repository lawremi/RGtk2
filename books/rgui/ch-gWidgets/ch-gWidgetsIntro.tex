...
\pkg{iWidgets} package of Simon Urbanek, initially implemented only
...
\pkg{qtbase}, and \pkg{tcltk}. The \pkg{gWidgetsWWW} provides a
...
\end{Schunk}
%
demonstrates the generic method \defn{\meth{svalue}} that can be used
to get or set the main property of the widget. For the object
\code{e}, the main property is the text, for the button and label
widgets this property is the label. The \meth{svalue\ASSIGN}
...
\args{index=TRUE} argument may be given to refer to the index, either
...
is a list with components \code{obj}, to pass in the object of the
...
a button. First the \pkg{gWidgest} package dispatches to a toolkit
...
(\pkg{tcltk}). Table~\ref{XXX} describes the installation process for
...
particular, the easiest to use (\pkg{gWidgetstcltk}) might have the
...
\begin{figure}
  ...
  \caption{Button layout for \pkg{RGtk2} (top), \pkg{tcltk} (middle) and \pkg{qtbase} (bottom). Although same code is used for each, the different styling yields varying sizes. }
...
When adding a child the \code{glayout} container should be specfied as
...
afterward through \method{addHandlerClicked}{gbutton}. The underlying
...
\argument{markup}{glabel} \code{TRUE} allows a native markup language
...
\begin{Sinput}
 ...
 makeColorIcon <- function(i) {
   filename <- sprintf("%s%scolor-%s.png", iconDir, .Platform$file.sep, i)
...
which can be set through the \argument{text}{gedit} argument.  If this
is not specified, and the argument \argument{initial.msg}{gedit} is,
...
values to be specifed through a character vector, as in \code{obj[]
  \ASSIGN\/ values}.
...
\argument{text}{gtext} argument is for specifying the intial text. The
...
argument \argument{use.togglebutton}{gcheckbox} is \code{TRUE}, a
toggle button -- which appears depressed when \code{TRUE} -- is used.
...
\code{index=TRUE} is specified. The data store is the set of labels and
...
\constructor{gcheckboxgroup} constructor. This convenience widget is similar to a radio group,
...
a similar functionality as using the \constructor{gtable} widget with multiple selection.
...
Comboboxes are constructed by \constructor{gcombobox}. As with the
...
\argument{by}{gslider}.  If \code{from} is a vector, then it is
...
\begin{Sinput}
 ...
 brightness <- gslider(from=-1, to=1, by=.05, value=0, handler=function(h,...) {
...
directory. For \code{RGtk2}, the \argument{filter}{gfile} argument,
used above, will filter
the possible selections. The dialog returns the path of the
file, or \code{NA} if the dialog was canceled. 
...
\argument{text}{gcalendar} arugment specifies the initial text. The
...
\code{expand=TRUE}. Otherwise, The size of the widget can be specified
by \meth{size\ASSIGN}. This can be list with components \code{width}
...
constructor's arguments \argument{chosencol}{gtable}, which specifies
which column value will be returned, as the user can only specify the
row; and \argument{multiple}{gtable} which controls whether the user
...
\code{filter.column} argument. In which case a combobox is added to
...
\argument{icon}{gaction}, \argument{key.accel}{gaction},
\argument{parent}{gaction} and the standard
\argument{handler}{gaction} and \argument{action}{gaction}. The label
...
\meth{tooltip\ASSIGN} method.) The key accelerator will bind a
keyboard shortcut, such as \kbd{Control-s} to an action. The
\code{parent} argument is used to specify a widget whose toplevel
...
\begin{example}{Popup menus}{ex-gWidgets-context-menus}
  This example shows how to add a simple popupmenu to a button.
\begin{Schunk}
\begin{Sinput}
...
between devices. A \code{ggraphics} instance can be made to represent
...
what is in common below.~\footnote{ For \pkg{gWidgetstcltk}, there is
  ...
  features. The \pkg{gWidgets} function merely wraps the
...
The \code{svalue} method returns name of the currently selected value
using \code{\$} to refer to child elements.  One can call
\code{svalue} on this to get the object.
