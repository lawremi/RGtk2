...
\begin{figure}
  \centering
  \includegraphics[width=.6\textwidth]{fig-qt-rstudio-like}
  \caption{A mock of for a possible GUI using dockable widgets and a \class{QMainWindow} instance}
  \label{fig:qt-r-studio}
\end{figure}


This example shows how to mock up a main window
(Figure~\ref{fig-qt-r-studio}) similar to the one
presented by the web application, R-Studio. (\url{rstudio.org}). 
