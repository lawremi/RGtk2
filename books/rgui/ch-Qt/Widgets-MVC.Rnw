...
The selection modes for item views are defined by the
...
\paragraph{Assigning the selection}
...
columns. In this example, we are interested in the rows the
user has selected:
...
\paragraph{The \signal{selectionChanged} signal}
...
\qcode{EditRole}, meaning that the data is in an editable form. 
...
A text cursor has a position and an anchor. The
...
can choose to move or keep the anchor in place. The motion of the
...
text.  Normally, the anchor and cursor are at the same position. To
