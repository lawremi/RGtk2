...
where the button object is specified for remove (not the index).


The \method{buttons}{QButtonBroup} method returns a list of the
managed buttons. Here we retrieve the buttons in the group and query
their checked state:
<<results=hide>>=
...
same layout, but the programmer is constrained to do this:
...
However, one need not do the internal management using a list, as
above, if a \class{QButtonGroup} instance is used to organize the
radio buttons (Figure~\ref{fig:qt-group-checkbox-radio}):
...
much easier to add (or remove) items, although you do need to add (or
remove them) to both the layout and the button group.
...
w <- Qt$QWidget()
lyt <- Qt$QHBoxLayout()
lyt$addWidget(make_cb())
lyt$addWidget(make_rb())
w$setLayout(lyt) 
...
\begin{figure}
  ...
  \caption{Two comobo boxes in a form layout}
...
le$text <- ""
le$setPlaceholderText("Enter some text")
@ 
