...
Next, we define the \prop{dataframe} property. When a new data frame
is set, we call the \meth{dataChanged} method to notify any views of a
change:
<<results=hide>>=
...
There are three virtual methods that we are required to implement:
\meth{rowCount}, \meth{columnCount} and \meth{data}. The first two
simply access the dimensions of the data frame:
<<results=hide>>=
...
represented in a column. We implement this with S3 methods, and
several are defined below:
