...
The allocation of space to child widgets depends on several factors.
...
values are flags andmay be combined with \OR{} to specify both vertical and
...
\ilayout{spring}\textit{stretch}. We add a stretch with a factor of $2.0$ and
...
box layout provides the \ilayout{strut}\textit{strut} for this purpose:
<<qt-layout-box-strut>>=
...
<<qt-layout-form-at>>=
quantileEdit <- flyt$itemAt(0, Qt$QFormLayout$FieldRole)$widget()
@ 
...
If the \property{checkable}{QGroupBox}
