...
bbutton$setSizePolicy(vertical = Qt$QSizePolicy$Fixed, 
...
<<addBox, results=hide>>=
layout$addLayout(sub_layout <- Qt$QVBoxLayout(), 1, 2, rowspan=3, 1)
...
layout$addWidget(label <- Qt$QLabel("More info:"), 3, 0, 
                 rowspan = 1, colspan = 2)
label$setSizePolicy(Qt$QSizePolicy$Fixed, Qt$QSizePolicy$Preferred)
label$setFrameStyle(Qt$QFrame$Box)
@ 
% 
...
column:
<<qt-layout-grid-at>>=
edit <- lyt$itemAtPosition(0, 1)$widget()
@
%
