...
sapply(justAction, justMenu$addAction)
@ 
%
...
window could list possible code completions (as seen in
Section~\ref{sec:multi-line-text}). There we implemented this logic in an override of the
