...
but all siblings in the widget hierarchy must be managed the same
...
There is no native calendar widget in \pkg{tcltk}. This example shows how one can use the validation framework for entry widgets to check that user-entered dates conform to an expected format. 
...
\TCL. (Otherwise the \code{ttktreeview}
widget will split values on spaces without quoting. One could use a
tclvariable here, but then extracting the string from the node becomes difficult)
\begin{Schunk}
\begin{Sinput}
...
\begin{Sinput}
 updateUI <- function() {
   ...
   tkentryconfigure(editMenu,"Undo", 
...
\begin{Sinput}
 ...
 for(i in seq_along(fontFamilies)) {
   fontName <- sprintf("::font::-%s", i)
