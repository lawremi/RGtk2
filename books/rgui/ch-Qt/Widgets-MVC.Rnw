...
perhaps edit the item's data within a view. Although custom delagates can be
defined, we are content here to use those provided by \Qt. 
...
describes the data for editing.  This allows, for example, a numeric item, to be
...
\class{QAbstractTableModel}, itself a subclass of
...
%% match and sort
The \method{match}{QAbstractItemModel} method can be used to return items in
...
\qtenumeration{Qt:::MatchFlag} enumeration, specifying how the matching is to
...
control its appearance. For example, columns (similarly rows) may be hidden
...
property. A \class{QItemSelectionModel} instance can return the current
...
model indices is returned. The methods \meth{row} and \meth{column} are
useful then.  The selection model allows one to specify if the selection
...
%% setting selection
The selection can be updated programatically through the
...
%% signals
The selection model emits a few signals, notably
