...
%% generator.
<<gtk-mvc-entry-buffer, eval=FALSE>>=
...
bounds$start$iter$getText(insertIter)
@ 
...
mark <- buffer$createMark(mark_name = "start", 
                          where = buffer$getStartIter()$iter, 
                          left_gravity = TRUE)
@ 
%
By setting \argument{left_gravity}{createMark} to \qcode{TRUE}, the
...
<<gtk-mvc-text-selectRange>>=
buffer$selectRange(buffer$getStartIter()$iter, 
                   buffer$getStartIter()$iter$forwardWordEnd())
@ 
...
buffer$pasteClipboard(cb, override_location = buffer$getEndIter()$iter, 
                      default_editable = TRUE)
@ 
%
The \argument{default\_editable}{pasteClipboard} indicates that the
pasted text should be editable. If we had passed \qcode{NULL}, to the
\argument{override\_location}{pasteClipboard} argument, the insertion
...
<<gtk-mvc-text-anchor>>=
anchor <- tb$createChildAnchor(buffer$getEndIter()$iter)
@ 
%
