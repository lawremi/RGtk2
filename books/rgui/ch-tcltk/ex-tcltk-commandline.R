...
tcl("set","data",data)
.Tcl('image create photo ::tcl::image1 -data $data -format "gif -index 0"')
.Tcl('image create photo ::tcl::image2 -data $data -format "gif -index 1"')


styles <- new.env()
addStyle <- function() {
  if(is.null(styles$Search.field))
    .Tcl(" ttk::style element create Search.field image {::tcl::image1 focus ::tcl::image2} -border {22 7 14} -sticky ew ")
  else
    styles$Search.field <- TRUE
  if(is.null(styles$Search.entry))
    .Tcl(paste(
               "ttk::style layout Search.entry {",
             "Entry.field -sticky nswe -border 1 -children {",
               "Text.padding -sticky nswe -children {",
               "Text.textarea -sticky nswe",
               "}",
               "}",
               "}",
               sep=""))
  else
    styles$Search.entry <- TRUE
  .Tcl("ttk::style configure Search.entry -background #b2b2b2")
  .Tcl("ttk::style configure -background #b2b2b2")
}
addStyle()
...
e <- ttkentry(f, style="Search.entry"); 

## text
textFrame <- ttkframe(f)
t <- tktext(textFrame)
## add scrollbars
tkpack(t)
## add tags
tktag.configure(t,"command", "underline"=TRUE)
tktag.configure(t,"output", foreground="blue")
tktag.configure(t,"error", foreground = "red")


tkgrid(e, row=0, column=0, sticky="ew")
tkgrid(textFrame, row=1, column=0, sticky="news")
tkgrid.rowconfigure(f, 1, weight=1)
tkgrid.columnconfigure(f, 0, weight=1)

tkbind(e, "<Return>", function(W) {
  val <- tclvalue(tkget(W))
  val <- paste(val, collapse=" ")
  out <- try(parse(text=val), silent = TRUE)
  if(inherits(out,"try-error")) {
    cat("parse error", out,"\n")
    return()
  }
  out <- try(capture.output(eval(out, envir=.GlobalEnv)), silent=TRUE)
  if(inherits(out,"try-error")) {
    cat("Error:", out,"\n")
    return()
  }
       
  if(val != hist[length(hist)]) {
    histp <<- length(hist) + 1
    hist[histp] <<- val
  }
    for(i in rev(1:length(out))) {
    tkinsert(t, "1.0", paste(out[i],"\n", sep=""), "output")
#    tkinsert(t, "1.end", "\n")
  }
  tkinsert(t, "1.0", paste(val,"\n", sep=""), "command")
  tkinsert(t, "1.0", "R: ", "command")
  tkdelete(W, "0","end")
})

tkbind(e, "<Up>", function(W) {
  if(histp > 1) {
    histp <<- histp - 1
    tkdelete(W,"0","end")
    tkinsert(W,"0",hist[histp + 1])
  }
})

tkbind(e, "<Down>", function(W) {
  if(histp < length(hist)) {
    histp <<- histp  + 1
    tkdelete(W,"0","end")
    tkinsert(W,"0",hist[histp])
  }
})
