...
The model, view, controller (MVC) pattern\iprogram{MVC} is fundamental to the design
...
simpler than with \GTK{} and \class{RGtkDataFrame} (See Section~\ref{sec:tabular-stores-tree}).
...
\index{qdataFrame}\function{qdataFrame}:
<<qt-mvc-qdfm-access>>=
df <- qdataFrame(model)
df[1:3, 1:10]
@ 
...
\class{QSortFilterProxyModel}.~\footnote{In \pkg{RGtk2} one can only
  choose one or the other, cf. Section\ref{sec:RGtk2:mvc:proxies}.) After constructing an instance and
